...
// 눌렸다 펴지며 출렁 복원한다 — 정지한 건축 실루엣 위에 통통한 두부 물리의 대비가 이 앱의
...
//   단, 지붕 창은 여기 적힌 값이 **상한**이다: 확정된 여행거리와 ROOF_CONTACT_MPS 에서 창을 파생하고
//   (끝은 고정, 시작만 늦어짐) 그 결과가 이 창보다 넓으면 이 창에 붙는다. 실제 종가는 [0.837, 1.00]
//   으로 좁혀져 공중 0.60s 다.
//   그래서 이 표의 숫자로 지붕 타이밍을 단정하지 말고 plan() 의 window/airborneSec 을 볼 것.
//   ★ walls 창 0.64 → 0.84 (2026-08-06, 사용자 지적 "반자 나오고 한참 뒤에 지붕이 나온다").
//   원인은 두 겹이었다. ① `brackets` 슬롯은 **어떤 빌더도 만들지 않는 그룹**이다(레포 전체에서
//   'brackets' 라는 이름을 붙이는 곳이 assembly.js 의 PART_ORDER 뿐) — 궁·절도 공포를 별도 파트
//   그룹으로 내보내지 않으므로 0.58~0.82 창은 모든 건물에서 빈다. ② 그 위에 지붕이 공중 창을
//   접촉속도에서 파생해 시작을 0.70 → 0.837 로 늦춘다. 두 개가 겹쳐 벽이 끝나는 0.64 부터 지붕이
//   내려오기 시작하는 0.837 까지 **아무것도 지어지지 않는 구간**이 생겼다 — 몸채 7.4s 기준 1.45s,
//   타임라인의 20% 다(실측 scratch/roof-chunk/probe.mjs).
//   지붕 쪽으로는 못 당긴다: 공중 시간이 이미 승인 밴드 상한(0.45~1.3s) 근처인 1.21s 다. 그래서
//   벽·창호가 지붕 하강 시작까지 계속 올라가도록 walls 창을 유령 슬롯 자리로 넓혔다. 앞쪽(빈 대지
//   데드타임)은 손대지 않았다.
//   `brackets` 는 PART_ORDER 에 남겨 둔다(없으면 건너뛰므로 무해하고, 훗날 공포를 별도 그룹으로
//   내보내면 그 자리에서 논다) — 다만 창은 walls 와 같은 구간으로 좁혀 죽은 공백을 다시 만들지 않는다.
const PART_WINDOWS = {
  podium:   [0.00, 0.26],
  columns:  [0.18, 0.48],
  walls:    [0.42, 0.84],
...
//   이 배수만으로는 부족하다: dropBase 는 layout.totalH 에 비례하고, #28 이후 buildHanok 은 실측
//   5.92m 를 달지만 그 값은 dropBase 하한 1.2 에 걸린다(5.92*0.13=0.77). 그래서 최종 여행거리는 ROOF_LIFT_OF_MASS 상한이
//   기하에서 정한다(종가 1.71m, 처마 피크 3.98m < 용마루 5.92m). 이 배수는 상한에 닿지 않는
//   큰 건물(궁 등)에서만 실제 값이 된다.
//   거리를 줄이면서도 **접촉 속도는 보존한다**(3.85 m/s): 하강 프로파일을 중력형(가속)으로 뒤집어
//   접촉 속도가 1.67배로 오르고(riseVelDown 참조), 공중 창을 확정 거리와 ROOF_CONTACT_MPS 에서
//   직접 파생하므로 이 배수를 어떻게 바꾸든 속도가 유지된다 — 두부 계약(무게=진폭 아님, 접근 속도)
//   이 그대로다. 즉 이 배수는 **상한에 닿지 않는 큰 건물의 여행거리**만 정하고, 속도는 못 흔든다.
...
//   **buildHanok(히어로 종가)은 layout 을 달지 않아** 폴백 12m 가 쓰였다(#28 에서 실측 5.92m 부착 —
//   dropBase 1.56 → 하한 1.2, 지붕 여행거리는 여전히 아래 상한이 정한다). 실제 종가 높이는 5.92m 이므로
...
//   상한이 걸리면 **접근 시간도 함께 줄여** 접촉 속도를 보존한다(무게=접근 속도 계약).
//   그래서 이 상한은 "느리게 둥실 내려오는" 쪽으로 새지 않는다 — 짧고 낮은 인양이 된다.
const ROOF_LIFT_OF_MASS = 0.45;

// 내려앉는 강체 지붕의 **목표 접촉 속도(m/s)**. 무게는 진폭이 아니라 접근 속도에서 오므로(①),
//   이것이 지붕이 얼마나 무겁게 도착하는지를 정하는 단 하나의 저작 값이다. 3.85 는 승인 밴드
//   (3.3~4.4)의 중앙이고 상한 미적용 큰 건물이 자연히 내는 속도(3.89)와도 같은 자리다.
//   종전에는 이 값이 저작되지 않고 `dropBase`(=layout.totalH 파생)에서 **간접적으로** 나왔다:
//   공중 창을 `상한/상한적용전_거리` 비율로 좁혔기 때문에, 상한이 거리를 1.71m 로 고정해도
//   속도는 상한 적용 **전** 거리에 비례해 흔들렸다. #28 에서 종가에 실측 totalH(5.92m)가 붙어
//   dropBase 가 1.56 → 1.20 으로 내려가자 여행거리·기하는 전혀 그대로인 채 접촉 속도만
//   3.83 → 2.95 m/s 로 떨어진 것이 그 증거다(2026-07-31). 이제 창을 거리와 이 목표에서
//   직접 파생하므로(want = CONTACT_V_DOWN·거리 / (목표·duration)) 속도는 dropBase·건물 높이·
//   duration 과 무관하게 일정하고, 상한이 거리를 깎으면 창도 정확히 그만큼 좁아진다.
const ROOF_CONTACT_MPS = 3.85;
...
export function playAssembly(building, { duration = 5, onDone, amp = 1 } = {}) {
  ...
  for (const name of PART_ORDER) {
    ...
    // 내려앉는 강체: 여행거리를 지붕 덩어리 높이에 비례하도록 깎고(ROOF_LIFT_OF_MASS), 그 **확정된**
    //   거리와 목표 접촉 속도에서 공중 창을 파생한다(속도 = 거리 / 시간). 창 **끝**은 고정이라
    //   정착 시점(=클라이맥스)은 그대로고, 줄어드는 것은 공중 구간뿐이다.
    //   창은 저작 창(PART_WINDOWS.roof) 안으로만 좁혀진다 — 앞으로 넓히면 벽 파트를 잡아먹고 전체
    //   duration 과 빈 터 dead time 이 늘어난다. 그래서 거리가 저작 창으로도 목표 속도를 못 낼 만큼
    //   길면(큰 건물) ws0 에 붙고 속도는 종전과 정확히 동일하다 — 상한 미적용 경로는 회귀가 없다.
    let massY = 0;
    if (rigid && drop < 0) {
      massY = localCenter(grp).y;
      const cap = ROOF_LIFT_OF_MASS * massY;
      if (massY > 0 && Math.abs(drop) > cap) drop = -cap;
      if (duration > 0 && Math.abs(drop) > 0) {
        const want = (CONTACT_V_DOWN * Math.abs(drop)) / (ROOF_CONTACT_MPS * duration);
        ws = Math.max(ws0, we - want);
      }
    }
