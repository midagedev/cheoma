...
export function buildVillageWall(shape, wallMats, opts = {}) {
  ...
  const M = wallMats;
  const shp = (shape && shape.pts && shape.pts.length >= 3)
  ...
  const gateEdge = Number.isInteger(opts.gateEdge) && opts.gateEdge >= 0 && opts.gateEdge < n
    ? opts.gateEdge : defaultGateEdge;
  const gateT = Number.isFinite(opts.gateT) ? Math.max(0, Math.min(1, opts.gateT)) : 0.5;
  ...
  if (style === 'open') {
    placeGatePosts(g, pts[gateEdge], pts[(gateEdge + 1) % n], gateT,
      gap, 1.25, M.mud, M.jipjul, 'brush');
  ...
  for (let k = 0; k < n; k++) {
    ...
    if (k !== gateEdge && ed && ed.share) continue;        // gate edge는 공유 담으로 생략하지 않는다
    const hK = k === gateEdge ? 1 : ed ? ed.heightK : roles[k] === 'front' ? 1
      : (roles[k] === 'back' ? 0.72 : 0.7);
    const H = baseH * hK;
    const a = pts[k], b = pts[(k + 1) % n];
    if (k === gateEdge) {
      buildGateEdge(g, style, coping, a, b, gateT, H, th, gap, cx, cz, M, rng);
    } else {
...
// 임의의 필지 변에 대문 폭만큼 구멍을 내고 양쪽 담 run과 문틀을 같은 축으로 배치한다.
function buildGateEdge(g, style, coping, a, b, t, H, th, requestedGap, cx, cz, M, rng) {
  ...
  const centerT = Math.max(halfT + 0.2 / length, Math.min(1 - halfT - 0.2 / length, t));
  const left = G.lerp(a, b, centerT - halfT);
  const right = G.lerp(a, b, centerT + halfT);
  if (G.dist(a, left) > 0.05) g.add(makeEdgeRun(style, coping, a, left, H, th, cx, cz, M, rng));
  if (G.dist(right, b) > 0.05) g.add(makeEdgeRun(style, coping, right, b, H, th, cx, cz, M, rng));
  if (style === 'brush' || style === 'hedge') {
    placeGatePosts(g, a, b, centerT, gap, H, M.mud, M.jipjul, 'brush');
  } else {
    placeGatePosts(g, a, b, centerT, gap, H, M.wood, M.woodDark,
      style === 'tile' ? 'tile' : 'stone', M);
  }
}
