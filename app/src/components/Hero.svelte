<script>
  // 진입 히어로 오버레이 — 한지 바탕 + 로고타입 cheoma + 낙관 악센트. 클릭/스페이스로 입장.
  // 브랜딩 다이어트(#72): 세로 '처마'·태그라인 제거, 로고타입 cheoma 만 담백하게. 안내만 로케일화.
  import { t } from '../lib/i18n.svelte.js';
  let { onEnter, leaving = false } = $props();
  function key(e) { if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); onEnter?.(); } }
</script>

<svelte:window onkeydown={key} />

<div class="hero" class:leaving onclick={onEnter} role="button" tabindex="0" aria-label="enter">
  <div class="title">
