...
| [`design-system.md`](design-system.md) | 계약 | Spectrum 기반 제품 크롬 재료 맵(목표→구성). 인스펙터/글래스/브랜드 표면 분리와 PropertyField 계약. |
...
- 외부 조사가 구현 판단에 쓰이면 같은 PR에서 관련 도메인 문서에 **원문 사실 / 제품 해석 / 한계**를,
  [`credits.md`](credits.md)에 기관·자료명·적용 위치·canonical URL·이용조건을 기록한다. 이 항목은 실제
  References UI에 노출하고 browser smoke에서 파싱된 적용 문구, `href`, `target`, `rel`을 확인한다. 새 링크는
  조사 중 브라우저로 직접 열어 유효성과 최종 도착지를 확인한 뒤 기록한다. 코드만 머지하고 출처를 후속으로 미루지 않는다.
