...
- 일반 필지 근경은 남측 일조축 안의 정확한 10° 카메라 고도와 1.65–2.5m 문 높이 조준을 유지한다. 7°/9°/12° 실앱 비교와 사용자 검수를 거쳐 9°보다 조금 높고 12°보다 건축적인 최종 값으로 정했다. 하늘을 위한 별도 수직 composition은 0으로 두어 집과 전경 마당을 화면 밖으로 밀지 않는다.
...
공유한다. 최초 등장은 70° 안의 완만한 establishing 호와 넓은 화각에서 출발해 정확한 10°·문 높이 망원
...
GitHub #15 근경 구도는 지붕 중심이 아니라 1.65–2.5m 문 높이를 조준하면서 카메라를 정확히 10°로 올린다. 3° 복원은 회귀 원인을 분리하기 위한 기준선이었지만 실제 제품에서는 담과 대문이 집·마당을 가렸다. 10°와 0 수직 composition은 종가 landing·직접 `집 보기`와 일반 기와·초가에서 집, 열린 마당, 대문·등롱, 생활 동물을 함께 확인하는 최종 계약이다. 종가 닭은 대문 바로 안쪽에서 열린 안마당으로 옮겨 두 제품 경로에서 ray와 100 이상의 pure-canvas 변경 픽셀을 만든다. 이 정책은 주거형 hero에만 적용하고 관아·궁궐형 hero에는 닭을 만들지 않는다. 주택 카메라의 XZ 시선은 필지의 `solarAccess` 안을 지난다. 이웃집은 실제 variant 지붕·지형 높이와 30° 저각 겨울 햇빛에서 1.5m 창 일조를 보장한다.
