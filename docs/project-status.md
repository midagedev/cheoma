...
장기 이슈 정리는 독립성이 확인된 코드·테스트·문서/리서치 청크를 별도 worktree와 서브에이전트에 병렬 배정한다.
각 청크는 겹치지 않는 소유 파일, 완료 조건, 빠른 게이트를 먼저 받고, 통합 담당은 결과를 그대로 신뢰하지 않고
diff·회귀·시각 결과를 다시 검토한다. 같은 파일·같은 hash baseline을 동시에 바꾸는 작업과 선행 설계에 의존하는
작업은 병렬화하지 않는다. 사용한 worktree와 임시 의존성 링크는 PR 머지 뒤 즉시 정리한다.
...
- 일반 필지 근경은 남측 일조축 안의 정확한 18° 카메라 고도와 1.65–2.5m 문 높이 조준을 유지한다. 10° 제품 프레임에서 마당과 생활 소품을 더 보여 달라는 연속 사용자 검수에 따라 12°·14°·16°를 거쳐 18°로 올렸으며, 별도 수직 composition은 0으로 두어 집과 전경 마당을 화면 밖으로 밀지 않는다.
...
- 공개 API: building, temple plan/full renderer, village plan/full runtime, environment, cinematic, audio, props, export, rendering lifecycle.
...
공유한다. 최초 등장은 70° 안의 완만한 establishing 호와 넓은 화각에서 출발해 정확한 18°·문 높이 망원
...
후속 GitHub #81은 아직 코드 구현 전인 야간 원경 불빛 계약이다. AURI가 설명한 실내측 한지 면과 외부에서
...
[`credits.md`](credits.md)에 함께 남겨 References UI가 같은 설명과 원문 링크를 보여 주게 한다. 예정된 순수/depth
fixture·앱 수명·고정 카메라 야간 PNG 검증은 [`verification.md`](verification.md)에 구현 완료 조건으로 기록했으며,
아직 통과 결과로 간주하지 않는다.
