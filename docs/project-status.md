...
- 마을 부감은 공간을 읽는 46° 광각, 일반 필지·hero·궁·사찰 근경은 형태와 배경 압축을 살리는 10°/7°/24°/26° 망원을 사용한다. 24mm 세로 센서 환산으로 일반 필지는 약 137mm, hero는 약 196mm라 200mm급 망원 인상을 낸다. 23°/21° reference FOV에서 보상 dolly해 피사체 크기를 유지하고, 디테일 LOD·입자 휴면은 화면 등가 거리로 계산한다. point sprite는 authored lens 중 가장 좁은 7°까지 전체 lens scale을 적용한 뒤 각 shader의 pixel cap으로 상한을 둔다.
...
- 근경 DoF는 단순한 흐림이 아니라 밝은 광원이 원형 aperture image로 읽혀야 한다. 피사체 축깊이, depth-writing mesh의 기여, 전환 중 단조로운 aperture를 자동 계약으로 보존한다. 주거 focus의 의미 초점은 카메라 조준점이나 움직이는 문짝이 아니라 고정된 주출입문 개구면 중심이다. overlay 진입·필지 hop·재건축·재굴림에서만 그 세계점을 원자적으로 갱신하고, focus-in은 조준점→문, hop은 문→문을 카메라와 같은 easing으로 잇는다. focus-out은 효과가 0이 될 때까지 출발 문을 고정한다. 주 전각이 불명확한 복합체만 기존 조준점으로 안전하게 물러난다.
...
- 공개 API: building, temple plan/full renderer, village plan/full runtime, environment, particles, cinematic, audio, props, export, rendering lifecycle.
...
fail closed한다. 기존 단일 `THREE.Points`의 `depthTest`를 켜 집·담·지형 뒤에서 가리며 color pass의
`depthWrite=false`를 유지한다. 근경 보케의 compact source transfer에는 같은 vertex shader·uniform·원형
discard를 공유하는 명시적 packed-depth material이 실제 창불 깊이를 제공한다. 따라서 일반 color scene은 계속
1 draw지만 DoF depth prepass에서 1 draw/1 program/1 material을 추가하며, triangle·texture·render target·pass는
늘지 않는다. 조사 근거와 제품
...
GitHub #96은 강수·꽃잎·낙엽·근경 mote의 재사용 경계를 `src/api/particles.js`로 모은다. Three 없는
강수 상태와 진행, 그 상태를 소비하는 물리 강수 표현, 근경 detail geometry, 제품 계절/mote runtime을
계층별로 노출하며 외부 소비자가 `src/env/` 내부 구현에 결합하지 않게 한다. seed가 정한 위치·위상·크기·종류
buffer는 하나뿐이고 한 개의 물리 월드 지오메트리가 이를 소비한다. color와 명시적 packed DoF depth는 같은
instance offset, 시간·바람·활성 uniform, 3축 회전과 alpha discard를 공유하므로 비활성 입자가 깊이에만
...
꽃잎 `2–5.4cm`, 가을 낙엽 `6–12cm`로 제한한다. 한지 창 HDR source는 이 입자 규칙과 마찬가지로 화면에
맞춘 임의 점을 만들지 않고 #81의 실제 고정 한지 opening anchor·외향 법선·mirror/fit/필지 변환과 정확한
