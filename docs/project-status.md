...
GitHub #15 근경 구도는 지붕 중심이 아니라 1.65–2.5m 문 높이를 조준하면서 기존 물리 카메라 높이는 보존한다. 주택 카메라의 XZ 시선은 필지의 `solarAccess` 안을 지난다. 이웃집은 실제 variant 지붕·지형 높이와 30° 저각 겨울 햇빛에서 1.5m 창 일조를 보장하고, 정자는 실제 지붕 면적으로 모든 필지의 남측 일조 통로와 focus 시선을 피하는 순수 배치 계약을 쓴다. 수치 계약과 기와집·초가·종가·궁·사찰 실제 앱 캡처를 함께 검증한다.

후속 GitHub #12는 [`temple-generator.md`](temple-generator.md)의 복합 가람 생성기로 진행한다. 현재 한 동짜리 빈 경내를 석등 몇 개로 임시 장식하지 않고, 순수 local-space 계획과 Three.js 조립을 분리해 작은 암자형부터 여러 전각의 중·대형 사찰까지 다른 프로젝트에서도 재사용할 수 있게 한다.
