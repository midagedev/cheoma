...
- 마을 부감은 공간을 읽는 46° 광각, 일반 필지·hero·궁·사찰 근경은 형태와 배경 압축을 살리는 10°/7°/24°/26° 망원을 사용한다. 24mm 세로 센서 환산으로 일반 필지는 약 137mm, hero는 약 196mm라 200mm급 망원 인상을 낸다. 23°/21° reference FOV에서 보상 dolly해 피사체 크기를 유지하고, 디테일 LOD·입자 휴면은 화면 등가 거리로 계산한다. 다만 먼 망원 카메라가 산을 통과할 때는 실제 렌더 지형 삼각형의 첫 안전 구간 안으로 거리를 연속 축소하고 actual FOV만 함께 넓힌다. 24° 고도·문 조준·화면 점유율과 reference FOV는 유지하며, 시선은 지형에서 1m, 카메라는 1.2m 이상 떨어진다. 30°를 넘는 예외는 진단하되 산속 카메라보다 집이 보이는 넓은 구도를 우선하고, 7° 최초 종가 진입은 일반 10°와 별도로 계산한다. 강수·꽃잎·낙엽·mote·한지 실용광은 실제 월드 크기를 가지며 authored camera projection으로만 화면 크기가 변한다.
