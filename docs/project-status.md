...
- 일반 필지 근경은 남측 일조축 안의 마당 눈높이(1.35m)에서 문 인방과 처마 사이를 조준해 처마와 하늘을 올려다본다. 비대칭 lens rise로 피사체를 화면 아래에 두되, 필지 크기·망원 dolly에 따라 카메라가 다시 부감으로 떠오르게 하지 않는다.
...
- 수묵 렌더: Three·DOM 없는 표현 상태, 재사용 `InkPass`, 앱 전환 runtime, Svelte 토글을 분리했다. 종이·normal target은 최초 진입 때만 만들고 완전 수묵에서는 중복 PBR post pass를 쉬게 하며, 단일 Output-last 색공간과 모바일·URL·접근성을 실제 앱 게이트로 고정했다.
...
GitHub #15 근경 구도는 1.35m 마당 눈높이를 유지하면서 건물 규모에 따라 3.0–5.6m의 문 인방–처마 대역을 조준한다. 주택 카메라의 XZ 시선은 필지의 `solarAccess` 안을 지난다. 이웃집은 실제 variant 지붕·지형 높이와 30° 저각 겨울 햇빛에서 1.5m 창 일조를 보장한다.
