...
> - **상태**: 현재 계약
> - **기준일**: 2026-08-08 (개정 — 방향 전환). 이전 기준일 2026-07-25.
> - **근거**: 사용자 결정 2026-08-08, 현재 코드, 영구 회귀 게이트

## 한 줄 방향

**2026-08-08 개정.** three.js 커뮤니티 공개 목표는 **달성됐다**. 현재 방향은 이 생성 엔진을 **다른 사람이 게임을 만들 때 가져다 쓸 수 있게 모듈화·툴화**하는 것이다. 계획은 [`packaging-plan.md`](packaging-plan.md)이며, 설계 중심축은 "최종 산출물은 구워진 메시가 아니라 **JSON 맵 계약**"이다 — 코딩 에이전트는 3D 씬을 볼 수 없지만 46KB 마을 plan은 읽는다.

**원 방향 (달성, 보존)**: cheoma의 목표는 공식 three.js 채널에 소개될 만한 완성도의 절차적 한옥 데모다. 기능 수보다 **코드 품질, 플래그십 룩, 한옥 품질, 대규모 마을, 릴리스 안정성**을 함께 보존하는 것이 우선이다. — 이 문장이 세운 품질 바는 여전히 기준이다. 달라진 것은 새 작업을 정당화하는 근거이지, 품질 하한이 아니다.
...
4. 골든아워 처마선과 한옥 근접 화면의 완성도를 높인다.
5. 대규모 마을의 측정된 병목이나 릴리스 회귀를 해결한다.
6. **외부 프로젝트가 이 생성 엔진을 소비하기 쉽게 만든다** (2026-08-08 추가). 패키지 경계·CLI·에이전트 스킬·반출 계약. 단 **생성 기능 자체를 늘리는 것은 여기 포함되지 않는다** — 아래 보류 항목 참조.
...
**2026-08-08 보완**: 이 보류는 방향 전환 뒤에도 유효하다. 모듈화·툴화(판단 기준 6)는 **기존 산출물의 포장**이지 생성 능력 확장이 아니다. [`packaging-plan.md`](packaging-plan.md) P0–P2는 새 생성 기능을 만들지 않고, P3만 신규 직렬화·반출을 추가한다. 생성기가 *만드는 것*을 늘리는 제안은 여전히 새 사용자 결정을 필요로 한다.

## 유지해야 할 결정

- 산의 빽빽한 숲은 한국 산의 핵심 룩이다. 성능 문제는 worker, instancing, LOD로 해결하며 숲을 잘라내지 않는다.
- 지형은 마을과 고정 버퍼까지만 만들고 `worldedge` 안개로 세계 끝을 마감한다(지형 radius 확장 금지).
  수평 `edge-mist-ring`은 아이레벨 전강도에서 아래보기 10°→30°로 연속 감쇠하되 **부감에서 0으로
  끄지 않는다**(U1 #211): capital/hanyang 의 scene fog near=`R*2.2` 가 terrainR 밖이라 절단면이
  fog에 안 걸리므로, 링 바닥 가중치(`EDGE_MIST_AERIAL_FLOOR`)·지형 edge haze·돔 지평 fog 수렴이
  하드컷 원반을 대기로 녹인다. 수직 `ridge-mist`도 부감 바닥값(`RIDGE_MIST_AERIAL_FLOOR`>0)을
  유지해 능선 겹침 몫을 남긴다. 링 `rIn`은 마을 바깥 밴드에만 걸려 분지 내부를 탁하게 덮지 않는다.
...
- 마을 부감은 공간을 읽는 46° 광각, 일반 필지·hero·궁·사찰 근경은 형태와 배경 압축을 살리는 16°/7°/24°/26° 망원을 사용한다. 24mm 세로 센서 환산으로 일반 필지는 약 85mm, hero는 약 196mm라 hero가 200mm급 망원 인상을 낸다. 일반 필지의 10°는 96m 보상 dolly에서 상단 프레임 광선이 −2°로 눌려 하늘·림이 성립하지 않아 16°로 재저작됐다(`src/camera/optics.js` VILLAGE_LENS 주석). 23°/21° reference FOV에서 보상 dolly해 피사체 크기를 유지하고, 디테일 LOD·입자 휴면은 화면 등가 거리로 계산한다. 먼 망원 카메라가 산을 통과하더라도 평소에는 거리를 줄이거나 actual FOV를 넓히지 않는다. 실제 terrain-grid 삼각형과 카메라를 향한 집 앞면 3×3 표본을 정확히 교차해 가장 깊은 전경 지형 뒤로 하나의 camera near plane을 옮기되, 가장 가까운 집 표면 앞에는 최소 1.2m를 남긴다. 이 투영 절단면을 focus-in·reveal·휠·복원 모두에서 최종 카메라 writer 뒤에 갱신하므로 색·물리 입자·DoF depth·숲이 같은 공간을 보고, 새 재질·pass·target·draw call은 없다. 절단면이 집에 닿는 예외에서만 실제 카메라를 첫 지형 안전 구간으로 당긴다. 따라서 결정론 회귀 필지 `capital/7/p47`(2026-07-31 재핀 — R2 필지 재배열로 p8 비무장)은 46.6m 거리와 16° 망원 압축을 유지한 채 약 13m 앞 능선만 자르고, 집 앞면은 약 47m에 남는다(9광선 전부 차단, minClearance −2.73m). 같은 프레임에서 그 절단면 앞에 걸치는 산 나무 인스턴스는 통째로 은닉해 땅 없이 떠 있는 수관을 없앤다. 7° 최초 종가 진입은 일반 16°와 별도로 계산한다. 강수·꽃잎·낙엽·mote·한지 실용광은 실제 월드 크기를 가지며 authored camera projection으로만 화면 크기가 변한다.
...
- 수묵(ink NPR) 표현 모드는 **2026-07-31 사용자 결정으로 제거**되었다. `?mode=ink`·`?clip=ink`·景/墨 토글·`src/render/ink*`·`src/api/render-style.js`가 모두 사라졌고, 제품에는 PBR 표현 하나만 남는다. 진경산수 리서치와 미학 원칙은 archive 문서 [`ink-landscape.md`](ink-landscape.md)에 역사 기록으로 보존한다.
...
필지의 남측 `solarAccess` 안에서 후보 세 개만 결정적으로 평가한다. 일반 경로는 기존점·중간점·중앙점을
유지하고, renderer 없는 마당 소품 anchor가 있으면 기존점·소품 방향 중간점·소품 방향점을 같은 ±14°
범위에서 평가한다. 두 경로 모두 기존 `1/0.9/0.8` 거리와 최대 20% 망원 보상 계약을 그대로 써 화면
점유율과 blocker 밖 안전 후보를 지킨다. 공간 그리드에는 picking용
...
건물이나 소품 내부로 되돌리지 않는다. 낮은 생활상은 선택 주택 지붕과 `wall-contract.js`의 실제 담
run·대문 개구도 별도 공간 인덱스의 first surface로 검사하며, 건축
가시성이 최선 후보보다 1/9 넘게 나빠지면 소품 점수가 카메라를 움직일 수 없다. 따라서 뒤안 소품 하나를
억지로 드러내려고 남향 접근을 깨거나 실제 지붕을 숨기지 않는다. 이 결과가 pick proxy의 권위 있는
...
GitHub #19 집 편집 패널은 “다시 보기”를 제거하고 집 주 액션과 `내보내기`만 남긴다. 리롤(`rerollParcel`)은 장식 애니메이션 재생이 아니라 원래 예약된 필지 안에서 경계·집 변주·담장·마당 살림·과실수를 하나의 트랜잭션으로 새로 구성하고, 결과는 focus-out 뒤에도 권위 있는 부감 오버레이로 남아 낡은 인스턴스가 되살아나지 않는다.

2026-07-31 #26 개정: 집 주 액션은 `고쳐짓기`(en `Renovate`)로 재작명됐고, 편집 가능 필지에서는 리롤 대신 **같은 집의 파라미터 라이브 몰핑**이다 — `app/src/lib/rebuild-morph.js`의 시드 결정 순수 plan(연속축 4–7 + 이산축 ≤2, 지붕→비례→스킨 창 스태거, smoothstep)이 기존 live-edit 프리뷰 경로로 흐르고 완료 시 커밋 1회. 마을·필지 시드는 불변이라 focus-hop/리롤 분리 계약과 무접촉이다. 라이브 rebuild 경로가 없는 비편집 필지(비병합 궁·절, ㅁ자집 등)만 위 리롤을 폴백으로 유지한다. 순수 게이트는 `check:rebuild-morph`, 브라우저 소유 게이트는 `check:parcel-rebuild:browser`.
...
GitHub #145는 초가 우진각 양 어깨에 검은 개구처럼 보이던 패치를 후처리나 지붕 텍스처가 아니라 민도리 상부 구조의 실제 교차로 분리했다. 창방·장여·도리를 `choga-frame-plan.js`의 한 수직 스택으로 계산해 도리 윗면을 지붕 전역 최저점인 처마선보다 2cm 아래에 두고, 세 부재는 서로 겹치지 않고 맞댄다. 3·4·5칸과 편집 한계·fuzz 형상에서 실제 삼각 지붕 표면과 최소 간격을 검증하므로 카메라나 `polygonOffset`으로 결함을 숨기지 않는다.

GitHub #146은 담 renderer 안의 FULL 전용 장식 별채를 필지 계획 계약으로 승격했다. 기존 확률을 다시
해석하거나 집을 축소하지 않고, Three 없는 `auxiliary-building-plan.js`가 실제 처마·필지·대문 접근·
자기/이웃 30° 일조·마당 hard object를 통과한 한 채의 local 위치·원래 크기·맞배지붕·처마 polygon을
불변 JSON으로 확정한다. 기와/초가 palette를 빌리는 공개 renderer는 3 mesh·48 triangle과 geometry
dispose만 소유하며 새 material·texture·program은 만들지 않는다. 제품은 승인된 모든 별채를 역할별
전역 root 하나로 병합해 FULL/MID/FAR에 같은 물리 silhouette를 유지하고, focus/edit/export 때 소유
필지 source range만 접는다. 편집 commit은 실제 편집 처마로 별채를 다시 계획한 뒤 flora·문 가림 record와
원자적으로 교체한다. 같은 방향·높이 volume은 초기/편집 후 focus camera solver에도 들어가 카메라가 별채
안에 놓이거나 주택 시야를 뚫지 않게 하며, 병합 전 임시 geometry는 예외 경로에서도 즉시 해제한다. AURI
근거는 몸채·부속채·마당의 공간 어휘에만 쓰며 확률·치수·clearance를 역사적
실측 분포로 표시하지 않는다는 한계를 `credits.md`와 제품 References에 함께 노출한다.
같은 worker 고정 cohort의 메인 대비 `(objects, triangles)` 변화는 village `(0, -76)`, town
`(+4, +110)`, capital `(+4, -154)`, Hanyang `(+4, -2,570)`이며, 여섯 FAR/MID/FULL 전환에서
program 증가는 `0`이다. 즉 별채를 물리적으로 유지하면서도 기존 장식 경로를 정리해 실질 장면 비용은
평탄하게 유지했다.
...
이 문서는 changelog가 아니므로 완료 항목 번호를 복제해 유지하지 않는다. Claude Code 메모리의 내부 작업 번호는
GitHub 이슈·PR 번호가 아니며, 서로 같은 번호로 간주하지 않는다. 병합 여부와 최신 완료 목록은 Git 이력과
GitHub를 사용하고, 후속 폴리시 범위는 GitHub #150을 기준으로 추적한다.

## 2026-07-26 세션 — 결정 변경과 현재 상태

이 날 사용자 결정이 두 건 **뒤집혔다**. 코드 주석과 게이트가 옛 규칙을 굳혀 두었으므로 날짜와 이유를 남긴다.

**1. 조립 탄성 — #126 「반동 제거」 철회.** 당시 거부한 것은 탄성 자체가 아니라 형태였다("일단 다
지어진 다음에 그 이후 영역만 한번 덜렁거리고 말더라"). 원하는 것은 "바닥에서 올라오는 그 가속도
그대로 푸딩 같은 느낌"이며 종가만이 아니라 **전 경로 공통**이고, 조립 애니는 **디테일할수록 좋다**.
구 반동이 겉돈 이유는 취향이 아니라 구조다 — 상승이 `easeOutCubic` 이라 접촉에서 속도가 정확히 0 이라,
뒤에 붙는 탄성은 물려받을 운동량이 없어 필연적으로 임의 진폭의 독립 진동이 된다. 그래서 진폭을 줄여
다시 켜는 것이 오답이고, 접촉 속도를 초기조건으로 받는 감쇠 스프링이어야 한다. 전체 길이는 늘려도
좋다(그 자체가 공유할 만한 클립 비트다) — 단 **빈 터 대기 시간은 늘리지 않는다**.

**2. 필지·마당 — 집을 줄이지 않는다.** 리드가 "집을 필지 대비 작게"를 레버로 제안했고 사용자가
기각했다: 집을 줄이면 근접 디테일이 줄어 목표 1을 해친다. 레버는 사이트 확대·필지 확대·마당을
파생 최소치로 만들기 세 개다. 다만 같은 라운드의 고증 조사가 사용자 전제("이 당시가 땅을 빽빽하게
쓰지 않았을 것")를 **뒷받침하지 않았고**, 마당 크기는 작업 면적 요구가 아니라 **채 크기의 종속 변수**
임이 확인됐다(`architectural-authenticity.md` §9). 결론은 유지되지만 근거가 바뀌었다 — 마당은
**L/H ≈ 2.55** 로 채에 비례시킨다. 이 기준은 집을 줄이지 않고도 성립해 위 결정과 자동으로 합치한다.

**3. 게이트 무게 — 순수 우선.** 사용자 지적으로 확립됐다. 논리·수학으로 판정 가능한 것을 헤드리스
브라우저로 증명하고 있었다. 규칙은 **"원인은 노드에서 단정, 브라우저는 효과 확인 1회"** 이고
`AGENTS.md`·`CLAUDE.md` 의 검증 절에 옮겼다. 부수 효과로 저장소 전역 브라우저 락 경합이 처리량
병목이던 문제도 완화된다.

**4. 배포 — main 푸시가 릴리스 행위가 되었다.** `.github/workflows/deploy.yml` 이 순수 계약을 돌리고
통과할 때만 Cloudflare Pages 로 배포한다. 시크릿 `CLOUDFLARE_API_TOKEN`·`CLOUDFLARE_ACCOUNT_ID`
필요. 브라우저 게이트는 CI에 넣지 않는다(전역 락·실제 GPU 백엔드 전제, 헤드리스 절대 시간은 근거가
못 된다) — 머지 전 로컬 실행으로 남긴다.

남은 소규모 후속 항목(회첨골 골 타일, capital 호수 밀도 판단, 낙엽 투광 재판정 등)은 유지보수자의 작업 보드에서 추적한다. 2026-07-26자 세션 인계 기록이던 루트 `HANDOFF.md` 는 서술 전부가 현행과 어긋나 2026-08-02 공개 정돈에서 삭제했다.
