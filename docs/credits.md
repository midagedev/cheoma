...
- **활용 / Use:** 일반 주택의 ㅡ·ㄱ·ㄷ 가중 레퍼토리, 살림집의 백골 목재·한지 창호, 칸에 종속된 문·창 의미 슬롯과 기능이 다른 문/창을 위한 경량 frame/threshold grammar, 부엌 아궁이→구들→굴뚝 관계의 근거. **창 하부 머름 apron/rail**은 window에만 두고 문 하부 청판 rail은 별도 `lowerPanel` 의미로 둔다. 문·창 개수 상한과 폭 배수는 역사적 빈도나 보편 비례가 아닌 **앱의 칸·충돌 안전 범위**이며, 같은 네 축을 FULL 초가·기와집 renderer와 한국어 편집기가 소비한다. 지역 사례를 전국 분포로 일반화하지 않고 ㅁ자는 참고만 하되 미구현 / Basis for the weighted single/L/U repertoire, neutral bare-timber/hanji openings, bay-bound semantic slots, lightweight frame/threshold detail, and the kitchen-hearth-to-ondol-to-chimney relationship. A meoreum belongs below a window, while a door's lower cheongpan rail remains a separate `lowerPanel`; opening count/width limits are application safety bounds rather than a historical frequency or universal proportion, and the same four axes feed the FULL choga/giwa renderer and Korean editor. Regional examples are not treated as a national distribution, and enclosed ㅁ plans remain reference-only (`docs/house-diversity.md`, `docs/architectural-authenticity.md`, `docs/exterior-detail.md`, `src/layout/residential-openings.js`, `src/builder/residential-opening-details.js`, `src/builder/opening-detail-plan.js`, `src/builder/kitchen-hearth.js`, `app/src/lib/edit-schema.js`).
...
- **활용 / Use:** primary 출입문의 문고리·돌쩌귀·경첩이라는 기능 어휘를 확인하되, 궁궐 장식판과 실측 치수는 민가에 그대로 복제하지 않는다. 민가·사찰의 보이는 철물은 작은 경첩 띠 두 개와 고리 하나만 남기고 pivot은 순수 anchor 데이터로만 유지하는 제품 해석 / Confirms the functional vocabulary of ring handles, pivots, and hinges for a primary entrance without copying palace plates or measured dimensions onto civilian houses. Visible civilian/temple ironwork is a product interpretation limited to two small straps and one ring, while the pivot remains pure anchor data (`docs/exterior-detail.md`, `src/builder/opening-detail-plan.js`).
