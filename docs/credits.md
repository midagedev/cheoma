...
- **활용 / Use:** 일반 주택의 ㅡ·ㄱ·ㄷ 가중 레퍼토리, 살림집의 백골 목재·한지 창호, 칸에 종속된 문·창 의미 슬롯과 기능이 다른 문/창을 위한 경량 frame/threshold grammar, 부엌 아궁이→구들→굴뚝 관계의 근거. **창 하부 머름 apron/rail**은 window에만 두고 문 하부 청판 rail은 별도 `lowerPanel` 의미로 둔다. 문·창 개수 상한과 폭 배수는 역사적 빈도나 보편 비례가 아닌 **앱의 칸·충돌 안전 범위**다. 지역 사례를 전국 분포로 일반화하지 않고 ㅁ자는 참고만 하되 미구현 / Basis for the weighted single/L/U repertoire, neutral bare-timber/hanji openings, bay-bound semantic slots, lightweight frame/threshold detail, and the kitchen-hearth-to-ondol-to-chimney relationship. A meoreum belongs below a window, while a door's lower cheongpan rail remains a separate `lowerPanel`; opening count/width limits are application safety bounds rather than a historical frequency or universal proportion, regional examples are not treated as a national distribution, and enclosed ㅁ plans remain reference-only (`docs/house-diversity.md`, `docs/architectural-authenticity.md`, `docs/exterior-detail.md`, `src/layout/residential-openings.js`, `src/builder/opening-detail-plan.js`, `src/builder/kitchen-hearth.js`).
