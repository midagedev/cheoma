...
> - **상태**: 현재 계약 + 구현 근거 리서치
> - **기준일**: 2026-07-31 (사대문 문루 rank 추가, 2026-07-22 최초 계약)
> - **범위**: 궁궐·관아 전각, 사찰 전각, 사대문 문루의 절차 단청. 특정 현존 건물의 복원도는 아니다.

## 1. 적용 경계

단청은 목재 보호, 결함 은폐, 건물의 권위와 종교적 장엄을 함께 나타내는 건축 채색이다. 이 생성기에서는 `palace`, `temple`, 그리고 도성 관영 시설인 `city-gate`(사대문 문루, §3.1 — 2026-07-31 사용자 승인)에만 적용한다. `giwa` 반가와 `choga` 민가는 단청 설정, CanvasTexture, 패널 제어를 모두 만들지 않는다. 이는 조선시대 일반 가옥의 고급 채색 제한과 레포의 기존 유형 경계를 함께 지키기 위한 계약이다.
...
| 사대문 문루 | 0.58 | 0.22 | 모로. 궁 아래 위계 + 풍우 노출로 도막이 더 바램 |
...
### 3.1 사대문 문루 (`city-gate`)

**사용자 승인 2026-07-31.** 그 전까지 단청은 궁·사찰 전용이었고 성문 문루는 채색 없는 목부재였다. 승인 범위는 **문루 단청뿐**이며 잡상·취두 같은 궁 지붕 장식은 계속 거부한다(`roofRank='city-gate'`, `check-roof-rank-contract`).

- **격식**: 모로. 성문은 궁 정전 아래 위계이고 사방이 풍우에 노출되므로 궁(0.68/0.28)보다 선명도·격식을 한 단 낮춘 0.58/0.22를 기본값으로 둔다. 격식 축 자체는 살아 있어 상위 값을 주면 금모로·금까지 올라간다.
- **bucket 분리**: source key가 `(style, …)`을 포함하므로 `city-gate:5:2:band`는 같은 버킷 좌표의 `palace:5:2:band`와 다른 캐시 항목이다. 성문 단청을 바꿔도 궁·사찰 Canvas 픽셀은 건드리지 않는다.
- **적용 부위**: 문루 **목부재만** — 기둥 머리초 띠, 창방·평방 밴드, 공포 힌트 소로 블록, 처마 밑 채색 띠. 석축(육축·여장·홍예)과 지붕 기와면은 단청 비대상이다.
- **재질 예산**: `makeCityGateDancheong()`이 band·square 두 재질만 소유하고 사대문 4기가 공유하므로 병합 후 **+2 드로우콜, +2 텍스처**다. Canvas source는 기존 LRU를 공유한다.
- **UV 제약**: 단청은 baked map이므로 **uv가 있는 부재(Box/Cylinder)에만** 붙인다. 성곽의 손으로 감은 석축·지붕 지오메트리는 uv가 없어(병합 시 0으로 채워짐) texel(0,0) 한 점만 샘플하므로 단청 대상이 될 수 없다.
- **거리 수렴**: 근경에서 뇌록 바탕 + 머리초가 읽히고, 중경에서는 밴드 텍스처가 밉맵 평균으로 뇌록 톤 띠로 수렴한다. 별도 LOD 지오메트리를 만들지 않는다.
- **불변**: `giwa`·`choga`는 이 확장 뒤에도 `resolveDancheong()`이 `null`을 돌려주고 단청 텍스처·패널을 만들지 않는다.
