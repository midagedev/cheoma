...
## 9. 가람 밀집과 단(段)·막돌 석축 (2026-08-05 구현)
...
- 병합 콜: compact 99 불변 · courtyard 115 → **113** · extended 141 → **139**, programs 7 불변. 단·석축은 이미 그려지는 재질만 차입하므로 +0 이고, 담 재해석이 −2 다.
...
**상태 (2026-08-05 §9 라운드 이후)**: ①③은 닫혔고 ④는 부분, ②⑤는 미해결이다. ②는 사용자 결정으로 이 라운드 범위 밖("배경을 맞추는것까진 무린거 같고 절의 구조는 비슷하게 나와야지"), ⑤는 별개 라운드다.
...
5. **전각이 전부 단층·비슷한 높이다.** 장안사 대웅보전은 **중층**이라 가람의 정점이 확실하다. 현재 주불전은 rank 4 단층(columnHeight 4.18 · podium 1.4)이고 부속과의 높이 차가 사진만큼 벌어지지 않는다. §9 의 단(段)이 주불전을 부속보다 1.9m 더 올려 정점은 뚜렷해졌지만, **중층 매스 자체는 미구현**이다.

남은 것
- 2는 절 자체가 아니라 **산 어깨 배후 식생·암반의 프레임 점유** 문제다(`forest.js` ecotone 쪽). 사용자 판단으로 범위 밖.
- 4 잔여는 단 상면 톤 변주(정점색) — 새 재질 금지.
