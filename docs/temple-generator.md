...
- **하층 지붕은 스커트 지붕이다 — 용마루가 없다** (2026-08-05 접합부 개정). 실측으로 확정된 결함: 하층이 완전한 팔작이라 용마루가 10.56m 인데 상층을 8.62m 에 앉혀 지붕 상부 경사면이 상층 벽을 관통했고(레이캐스트 z=0 에서 3.60m 초과), 그 틈이 갈색 판으로 읽혔다. 실물 중층(화엄사 각황전·무량사 극락전)의 하층 지붕은 상층 벽에 닿는 얕은 스커트라 용마루가 없다. 제품은 **하층 물매만 0.22배**(0.68→0.15)로 낮춰 용마루를 상층 몸체 안(8.17 < 상층 벽 상단 9.9)에 잠기게 한다. 처마 곡선·겹처마·공포는 불변이고, 처마 외연이 물매와 무관하므로 §9.1 가람 밀집 계약도 불변이다.
- **상층은 원래 물매를 지킨다.** `upperStorey.roofPitch` 가 하층을 낮추기 **전** 값을 보존한다 — 낮춰진 하층 프리셋을 상속하면 상층 지붕까지 평평해져 가람의 정점이 사라진다(게이트 관계 ⑨).
- 0단 기단(`podiumTiers 0`)이면 **계단·소맷돌·난간도 없다**(`src/builder/podium.js`). 종전에는 tier 루프만 건너뛰어 전면 계단이 하층 지붕 위에 떠 있는 석판으로 렌더됐다.
- 층간 판벽 스커트(`compound.js#storey-skirt`): 4면 판 + 인방 2 + 칸마다 기둥. 재질은 상층 자신의 `woodDark`/`wood` 를 빌리므로 **새 재질군 0**(병합 콜 flat 147 · mountain 143 불변).
