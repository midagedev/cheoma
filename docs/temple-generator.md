...
## 9. 사료 사진 대조 갭 (2026-08-05 실측, 미해결)
...
1. **가람이 너무 벌어져 있다.** 사진의 전각들은 지붕이 거의 맞닿을 만큼 붙어 용마루 방향이 채마다 달라 지붕면이 조각보로 얽힌다. 현재는 55.8×59.0m 일곽에 전각을 벌려 놓아 마당이 화면의 절반을 먹는다.
...
3. **일곽이 정연한 직사각형이다.** 사진은 높이가 다른 막돌 석축 단이 느슨하게 감싸는 형태고 담이 한 겹 회랑으로 돌지 않는다. 현재는 균일한 `precinct-wall` 사각형이라 소규모 궁으로 읽힌다.
4. **마당 지면이 무텍스처 단색 평면이다.** 사진의 마당은 흙바닥에 단이 한 번 꺾이고 계단·석축이 끼어든다.
5. **전각이 전부 단층·비슷한 높이다.** 장안사 대웅보전은 **중층**이라 가람의 정점이 확실하다. 현재 주불전은 rank 4 단층(columnHeight 4.18 · podium 1.4)이고 부속과의 높이 차가 사진만큼 벌어지지 않는다.

닫으려면 필요한 것(범위 판단은 사용자 결정 사항 — 프로젝트는 마무리 국면이다)
- 1·3·4는 `plan.js`/`compound.js` 의 배치·일곽·마당 계약을 건드리므로 별개 라운드다.
- 2는 절 자체가 아니라 **산 어깨 배후 식생·암반의 프레임 점유** 문제다(`forest.js` ecotone 쪽).
- 5는 중층 전각 문법 신설이라 가장 크다. `roof.js`/`role-hierarchy.js` 에 새 massing 등급이 필요하다.
