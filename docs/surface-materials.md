...
당시의 수묵 트랙(2026-07-31 제거)이 근접 프레임에서 "지붕이 벽보다 밝게 읽힌다"를 관찰하고 그 원인을 albedo로 가정했다가, 실측으로 **가정이 틀렸음을 확인**했다. 관찰 경로는 사라졌지만 albedo 위계와 오진 방지 교훈은 PBR에도 그대로 적용된다. 그 실측을 여기 남긴다. 이 절의 목적은 값을 기록하는 것이 아니라 **같은 오진이 반복되는 것을 막는 것**이다.
...
### 판정 3 — 지붕이 벽보다 밝게 읽히는 원인은 albedo가 아니라 조도다

지붕은 상방을 향한 큰 면으로 하늘 반구 전체와 직사광을 거의 정입사로 받고, 벽은 수직면이라 저각 태양에 스침 입사이며 깊은 처마 아래 자기 그늘에 든다. albedo 2.5배 열세를 조도가 뒤집는다. 화면 휘도를 정규화하는 어떤 후처리도 그 차이를 한 번 더 벌린다(제거된 수묵 pass의 농담 입력 정규화가 그 극단 사례였다).

**회화는 태양이 없어 화원이 재질 정체성으로 톤을 배정하지만, 스크린 스페이스 후처리는 조도 결과만 본다.** 재질 기준 톤 배정을 하려면 per-object role 채널을 화면 pass로 실어보내야 하고 그것은 프로그램 축 +1이므로(`look-audit-2026-07.md` R8 미해결) 채택하지 않았다. 즉 이것은 **스크린 스페이스 톤 판정의 구조적 한계**이며 팔레트 결함이 아니다. 이 한계를 다뤘던 [`ink-landscape.md`](ink-landscape.md) 미학 원칙 8("단계 수"가 아니라 "씬 휘도가 그 단계를 점유하는지")은 수묵 모드와 함께 archive로 넘어갔고, 현행 판정은 [`look-grammar.md`](look-grammar.md)의 농담·여백 축이 맡는다.
