...
- `npm run check:nightlights`는 실제 초가·기와 ㅡ·ㄱ·ㄷ prototype에서 수집한 38개 anchor의 opening id,
  ...
  wave·dispose 수명과 `depthTest=true`/color `depthWrite=false`를 검사한다. 단일 Points의 color 예산은
  1 draw이고, 같은 vertex shader·uniform identity·원형 discard를 공유하는 packed-depth material이 DoF
  prepass에 1 draw를 더한다. 합계는 0 triangle, 2 material, 2 program, 0 texture, 0 light이며 두 재질은
  각각 정확히 한 번 해제된다.
...
| 실제 창호 야간 불빛 (#81) | [`exterior-detail.md`](exterior-detail.md) §4.2 + `check:nightlights` 실제 color/packed-depth 재질·공유 uniform/vertex·원형 discard·순수 anchor·자원 fixture + `check:worker` scene bytes + `check:parcel-rebuild:browser` owner 복원 + `check:app` 항목별 References UI + `shoot:nightlights` 고정 aerial/MID/focus·같은 카메라 depth A/B PNG + color 1 draw 및 기존 DoF target에 source-depth 1 draw |
