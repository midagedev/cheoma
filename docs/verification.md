...
village runtime, environment, post를 직접 구성하지 않고 `src/api/building.js` 하나만으로 건물을 생성할 수 있는지 검사한다. 문서의 CDN import
map은 `three@0.185.1`로 고정하되, 자동 smoke는 동일 URL을 로컬 설치본으로 fulfill하여 네트워크에 의존하지 않는다.

검사는 소비자와 코어의 `THREE.REVISION`·`instanceof` identity, 유한 bounds, 실제 draw/triangle 제출, 공유
`P.mats`의 borrower 소유권, 여섯 번 rebuild 뒤 geometry/texture plateau를 확인한다. 마지막으로 건물과 예제가
