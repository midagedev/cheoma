...
npm run shoot:focus-level   # 마당 저각 주택·종가·궁·사찰 실제 앱 구도 5장
...
- 1.35m 마당 눈높이에서 인방–처마 target을 올려다보고 -18% lens rise를 합성해 능선 위 하늘 띠를 확보하는지 검사한다.
...
- 일반 주택과 종가의 화면 중심은 건물 크기에 따라 3.0–5.6m 인방–처마 대역을 조준하고, 카메라는 지면 1.35m를 유지한다.
