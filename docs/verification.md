...
PR을 머지하기 직전 최종 변경이 끝난 동일 HEAD에서 `npm run check:full` 성공 1회를 남긴다. 이후 diff가 바뀌면 다시 실행한다. 이는 전체 browser-free 계약과 app/Worker/audio/
...
npm run check:all             # core/app/Worker/audio/temple/parcel/surface profile
...
CHEOMA_BROWSER=chrome npm run shoot:bokeh # 원형비·pan→settle·hardware GPU query fixture
npm run shoot:focus-level   # 22° 주택·종가·관아 hero·궁·사찰 실제 앱 구도 6장
...
`shoot:lod-transition`은 고정 Hanyang `p13` 청크를 실제 제품의 22° focus ray에서 바라보며 두 경계의
...
- `npm run check:nightlights`는 실제 초가·기와 ㅡ·ㄱ·ㄷ prototype에서 수집한 38개 anchor의 opening id,
  ...
  wave·dispose 수명과 `depthTest=true`/`depthWrite=false`를 검사한다. 단일 Points 예산은 1 draw, 0 triangle,
  1 material, 1 program, 0 texture, 0 light다.
- `npm run shoot:nightlights`는 실제 Vite 앱의 고정 seed·밤 상태에서 aerial/MID/focus와 같은 카메라의 정면,
  후면 depth-on/depth-off PNG를 OS 임시 디렉터리에 만든다. depth A/B는 뒤쪽 불빛이 불투명 집·담에 실제로
  가려지는지 픽셀로 판정하고, focus/rebuild는 같은 opening id의 이동과 geometry/material·owner slot 재사용을
  검사한다. 에이전트는 실제 한지 면의 절제된 따뜻한 bloom/bokeh, 어두운 방의 공존, 자유 부유점 부재를 직접 본다.
...
- `check:dof`
  ...
  - 이동 부분집합이 중심+세 반경의 13개 대칭 표본을 쓰고, 단일 uniform branch에서 일반 픽셀 13/HDR 보존·정착 픽셀 41 fetch로 갈리며 stable=1은 full 결과를 직접 반환하는지 검사한다.
...
- 정확한 22° 카메라 고도에서 1.65–2.5m 문 높이 target을 바라보고 별도 하늘 편향 없이 전경 마당을 화면에 남기는지 검사한다.
...
- 일반 주택과 종가의 화면 중심은 지붕 높이가 아닌 1.65–2.5m 문 높이 범위를 조준하고, 안전 가시성 dolly 뒤에도 카메라는 그 target을 향한 정확한 22° 고도를 유지한다.
...
| `tools/check-dof.mjs` | 축방향 초점, 단일 DoF controller, 깊이 기여 분류, 프레임레이트 독립 품질 상태·숫자 camera snapshot, 단일 프로그램 13/41-tap 원형 kernel의 순수 계약 | 실제 depth prepass와 전환 배선은 앱 게이트가 맡는다. |
...
| `tools/shoot-bokeh-fixture.mjs` | 실제 composer 위 제어 HDR 광원의 원형비·방사 균일도, 8단계 adaptive 패닝→hold→41-tap 정착 strip, 최종 기준 최대 채널 차이 1, Chrome hardware GPU timer query를 계측·촬영 | `CHEOMA_BROWSER=chrome`과 비-software renderer를 강제한다. 중앙 depth gather 때문에 초점면 불투명 표면과 겹친 전경 광원은 퍼지지 않는 한계를 별도 probe로 기록하며 GPU 성능은 wall time으로 판정하지 않는다. |
...
| `tools/check-lod-app.mjs` | `focus/wave/full` 선택 실행: Hanyang screen-door channel/자원·focus/hop/out, 실제 카메라 -Z 기반 수평 운해의 두 방위 중간각·수평/22° 끝점·자원 plateau·dispose, town→village fast wave 또는 Hanyang 전체 연속 흐름 | 빠른 wave가 Hanyang multi-chunk full을 대체하지 않아 merge 전 `check:full`을 유지한다. |
| `tools/shoot-lod-transition.mjs` | Hanyang p13의 실제 22° FAR↔MID↔FULL 전후/중간, 수평 운해 부감 0·수평 복원, identity/program/draw/triangle/pass parity | 고정 seed·카메라 한 세트이며 모든 방위·시간대의 운해 미감을 대신하지 않는다. |
...
| `tools/check-rim-facing.mjs` | 실제 WebGL에서 N·V 실루엣, 실제 태양 N·L, V·L 역광, directDiffuse=0·그림자·제품형 비그림자 fill·`receiveShadow=false` 반례, 50% penumbra 비율, 실제 cloud-shadow callback/cache-key 합성, LOD/일반 cloud+snow+rim 프로그램 분리와 동차 `worldPosition` 복원, 추가 shadow fetch 0, HDR 에너지 상한, instanceColor·program plateau; `--app`은 시간을 멈춘 최종 22° 석양 focus에서 실제 마을 cloud+rim 지붕과 OFF/ON A/B | 고정 fixture와 대표 실앱 한 장이 모든 seed·재질의 미학을 대신하지 않는다. |
...
| 실제 창호 야간 불빛 (#81) | [`exterior-detail.md`](exterior-detail.md) §4.2 + `check:nightlights` 재질·순수 anchor·자원 fixture + `check:worker` scene bytes + `check:parcel-rebuild:browser` owner 복원 + `check:app` 항목별 References UI + `shoot:nightlights` 고정 aerial/MID/focus·같은 카메라 depth A/B PNG + 기존 단일 Points GPU 0-delta |
