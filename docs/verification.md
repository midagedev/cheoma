...
디렉터리에만 남기고 수치 PASS만으로 미감을 승인하지 않는다. 2026-07-23 구현 전 read-only 기준은 p27,
1600×900 DPR 2에서 position·quaternion·FOV·projection을 고정한 채 조준 깊이 28.2073m와 개구면 깊이
25.9377m를 비교했고, 문 주변 crop의 RGB>1 변경 41.4%와 개구면 쪽 edge energy +18.7%를 기록했다. 이는
효과가 계측 가능한지 확인한 기준선이며, 최종 제품 판정은 같은 fixture의 구현 후 재촬영과 직접 이미지 검수로 한다.
