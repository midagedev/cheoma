...
npm run check:all           # check/app/petals/winter/worker/audio/temple/parcel browser 통합 게이트
...
- 전환 mix가 단조롭고 Ink가 단일 `OutputPass` 바로 앞에 있으며, 완전 수묵에서 grade·bloom·Bokeh·flare가 모두 휴면하는지 검사한다.
- shader program 증가는 5 이하, 축소 normal pass의 draw call은 원 장면 이하로 제한하고, 장식·대기 객체가 가짜 깊이를 만들지 않는지 확인한다.
...
| `tools/check-building-clearance.mjs` | 기초 매입, 마당 lift, ㄱ자 기단 단일 depth owner, 판벽 봉창 face clearance, 맞배 벽 tuck | 실제 접지선·기단 줄눈 미감은 기와 격리 하네스와 `layout.html` 필지 4종을 직접 본다. |
