...
| `tools/shoot-layout-solar.mjs` | 실제 앱의 부감·공중·기와/초가 focus, `frontDir`·`solarAccess`·보호수 overlay와 가림 진단 | seed·규모·시간·계절·날씨를 `CHEOMA_LAYOUT_*`로 고정한 시각 검토용이며 자동 미감 판정은 아니다. PNG는 OS 임시 폴더에 쓴다. |
