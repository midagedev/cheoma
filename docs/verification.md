...
| `tools/check-walk-control.mjs` | 1인칭 **수동 조작** 계약(#33, 순수): 진입 기본이 정지(M1)이고 자동 산책은 명시 호출로만 켜진다(M11). 전진 명령의 이동 거리가 램프 포함 해석해와 ±1mm 로 일치하며(M2), 정지 명령 후 걷기 0.10s·달리기 0.16s 안에 속도가 **정확히 0** 이 되고 그 뒤 위치가 변하지 않는다(M3). 대각 입력 정규화(M4), 픽셀 감도(yaw 0.0026·pitch 0.0022 rad/px)와 피치 클램프 ±68.75°(M5·M6), 시선 델타의 **1프레임 1회 소비**(M7 — 드래그 한 번이 무한 회전이 되던 배선 결함의 회귀 게이트), 회전의 즉시 반영(M8), 경사 지형 눈높이 오차 <0.05m·침하 0(M9), 실제 village plan 위 120초 주행에서 충돌 0·경계 이탈 0·터널링 0(M10), `update(dt,input)` 하위 호환(M12). R1 은 스텁 카메라·컨트롤로 `cinematic-runtime` 을 실제로 돌려 워킹뷰 진입 기본이 수동인지, 무입력 정지·명령 해제 정지가 성립하는지 본다. FAIL-first 확인(2026-08-01): M2·M3·M5·M7·R1 은 변경 전 HEAD 소스에서 실제로 실패, M1·M8 은 HEAD 에서도 통과(각각 코어 재발 방지·전방 보호막) | 렌더·DOM·키보드 이벤트 수집은 다루지 않는다(입력 수집은 `app/src/App.svelte` 소유, 브라우저 확인은 `check:cinematic:app`·수동 캡처). 자동 산책 경로의 품질(밀집 스팬·담장선 접근·스폰 밀도)은 `check-cine-quality.mjs` W1~W6 이 계속 소유한다. |
