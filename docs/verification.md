...
Svelte·마을·post 없이 공개 `src/api/building.js`만 사용해 기와집과 초가를 각각 여섯 번 생성·렌더·해제한다.
GPU texture·geometry와 draw call·triangle 수가 반복 후 plateau인지, 해제된 기와 texture가 Three r185의 공유
shadow depth material을 통해 다시 WebGL에 등록되지 않는지, owned texture가 정확히 한 번 해제되는지를 실제
Chrome/ANGLE에서 검사한다. 공유 `P.mats` borrower는 호출자 재료를 보존하고 owner만 최종 해제해야 한다.

opaque stock caster에 불필요하게 복사된 `map`/`alphaMap`만 shadow draw 직전에 지운다. `alphaTest` silhouette과
`customDepthMaterial`/`customDistanceMaterial`의 authored texture는 보존한다. 양 방향 hook 설치 순서에서도
LOD screen-door matrix channel과 기존 `onBeforeShadow`가 유지된다. 이 게이트는 `check:all`/`check:full`에 포함되며 builder, 공개 building API,
shadow lifecycle과 LOD screen-door 변경에서 `check:pr`가 자동 선택한다.

### `npm run check:api-reuse`

하나의 browser lane에서 세 개의 좁은 소비 경로를 순서대로 검사한다. 먼저
[`examples/api-building/`](../examples/api-building/)을 실제 정적 페이지로 열어 외부 소비자가 Svelte 앱,
village runtime, environment, post를 직접 구성하지 않고 `src/api/building.js` 하나만으로 건물을 생성할 수 있는지 본다.
이어서 공개 `src/api/sijeon-plan.js`와 `src/api/sijeon.js`만 사용하는 대표 시전 거리 프레임에서 borrowed-material
renderer와 dispose를 확인하고, 마지막으로 실제 Vite 한양 앱의 제품 카메라·view shift·post·부감 림 정책에서 데스크톱/모바일 시전 픽셀 기여를 A/B로 증명한다.
문서의 CDN import map은 `three@0.185.1`로 고정하되 자동 smoke는 동일 URL을 로컬 설치본으로 fulfill하여 네트워크에 의존하지 않는다.

검사는 소비자와 코어의 `THREE.REVISION`·`instanceof` identity, 유한 bounds, 실제 draw/triangle 제출, 공유
`P.mats`와 시전 다섯 재질의 borrower 소유권, 여섯 번 rebuild 뒤 geometry/texture plateau를 확인한다. 마지막으로 건물과 예제가
소유한 renderer/canvas/resize listener/debug hook을 해제하고 이중 dispose와 종료 후 rebuild가 안전한지도 검사한다.
이 게이트는 `check:all`/`check:full`에 포함되며 예제 entry의 실제 정적 ESM dependency가 바뀌면
`check:pr`가 자동 선택한다. 실행과 외부 통합 계약은 [`external-reuse.md`](external-reuse.md)에 정리한다.
...
- `check:architecture`
  ...
  - 최소 외부 예제의 저장소 import는 `src/api/building.js` 하나로 한정하고 import map의 Three 버전을 고정.
...
- `check:celestial`
  - 태양 원반(림 다크닝·저고도 편평화/확대), 달 위상(조명 방향·가시 조명 면적·터미네이터·지구조), 별(시드·등급 멱법칙·반구·시간대 페이드), 은하수, `SUN_BAND` 밤 배율과 `sky.js` 배선 정합을 검사한다. 순수 노드(브라우저 없음).
- `check:cloud-silhouette`
  - 구름 아틀라스 실루엣의 위상 무결, 패밀리 비유사도, 미러 정체성, 씬 배치 위계, 방향 반복, wisp 외곽 꼬리와 드로우 예산을 검사한다. 순수 노드(자체 esbuild 번들, 브라우저 없음).
- `check:cloud-fade`
  - 부감에서 페이드 아웃되는 상공 구름 빌보드가 HDR 림·역광 투과 잔존으로 "펜선" 윤곽만 남지 않는지 검사한다(페이드가 opacity뿐 아니라 림/역광 게인에도 걸림 + 하늘 ROI 픽셀 잔여 구조). 브라우저(capital 부감 sunset 픽스처).
- `check:plan`
  ...
  - 입력 옵션 불변.
- `check:temple`
  - compact/courtyard/extended의 최소·최대 크기 × 3 seed, 총 18개 순수 계획의 반복 결정론과 전역 `Math.random` 미사용을 검사한다. 각 전각의 역할 위계, seed-stable 지붕·공포 조합, 처마·기둥·기단 단계와 실제 처마 polygon도 같은 plan에 완결되는지 확인한다.
  - 담장·남측 산문·마당·전각·의미 기반 석조물, 실제 처마 footprint 내부성, 전각/프롭 겹침과 주불전 남측 일조 통로를 검사한다. 정자를 통로 안에 고의로 넣은 반례가 반드시 실패해야 한다.
  - architecture 필드가 없던 v1 fixture가 입력 불변·결정론적으로 canonical v2와 같아지는지, 불완전한 v2와 누락·미지원·미래 schema가 renderer 전에 명시적으로 거절되는지 검사한다.
  - solo/hamlet/village/town/capital/hanyang adapter가 실제 폭·깊이를 예약하고 남측 문에서 도로·성문 또는 road-free solo 중심으로 이어지는지 검사한다.
  - 송림사 맞배·다포와 부석사 무량수전 팔작·주심포 근거가 도메인 문서·credits #39·Product References 파싱 경계 안에서 자체 URL·적용 한계·라이선스와 함께 남는지 검사한다.
- `check:roads`
...
- `check:critters`
  - 기러기 V자 편대(skein) 슬롯 기하를 검사한다 — 선두 1 + 두 팔 echelon, 어떤 추종 개체도 앞 개체의 정후방에 서지 않음, 같은 팔의 간격·각도 범위, 개체 겹침 0, 계급이 깊어질수록 벌어지는 V. 반각은 41초 주기로 천천히만 변한다.
  - 편대 적분기가 200초에 1.5km 이상 하늘을 가로지르고(제자리 선회 금지), 선두를 교대하며(슬롯 배정은 항상 순열), 무리 전체 롤 부호가 하나이고 뱅크가 정책 상한 안인지, 같은 seed가 byte 단위로 재생되고 전역 `Math.random`을 소비하지 않는지 검사한다.
  - 계절 게이트가 가을·겨울에만 기러기 편대를, 봄·여름과 알 수 없는 값에는 텃새 무리를 고르는지 검사하고, 실제 렌더러에서 계절 전환 뒤 인스턴스 위치가 진행 방향 기준 좌우 두 팔로 갈리는지(선두보다 앞선 개체 0) 검사한다.
  - 개체수가 규모 고정값이 아니라 필지 밀도로 정해지고 상한에서 멈추는지, 5개 규모 × 3개 seed의 실제 plan에서 시선-셀 LOD 창(spatial full 32m / hidden 72m) 안에 개·고양이를 갖는 필지 비율이 기준선을 넘는지 검사한다. 종전 고정 상한(마을 전체 개 2)은 이 기준을 통과하지 못해야 한다(반례 고정).
  - 개 순찰 구간과 고양이 페르치가 `villageWallLayout` 실측값에서만 나오는지 검사한다 — 구간 전체가 필지 안·담 이격·몸채 벽면 밖이고(0.25m 간격 표본), 담 위 고양이의 높이가 담 높이 계약값과 같고 대문 개구 안에 앉지 않으며 담 선 위에 있다. 종전의 반경 6.5m 랜덤워크는 이 계약을 통과하지 못해야 한다(반례 고정).
  - 실제 렌더러가 종별 단일 InstancedMesh(≤4 드로우 오브젝트)·지상 공유 재질 1벌·텍스처 0을 유지하고, 다리·꼬리·머리 관절이 정점 속성(`aPivot`/`aSwing`)과 인스턴스 속성(`aAnim`)으로만 돌아가며, 전역 `Math.random`을 오염원으로 바꿔도 배치·애니메이션이 byte 동일한지 검사한다.
- `check:parcel-rebuild`
  - 전 규모 99개 재건축에서 같은 seed 결과, 불변 원본 envelope, 누적 축소 없음, 실제 처마 fit, 남향·도로 접근·일조 계약과 별채 spec 결정성을 검사한다.
...
- `check:lod`
  ...
  - 필지마다 안정 시 원경 mass·중거리 envelope·전체 인스턴스·선택 overlay 중 정확히 하나, 이행 시
    FAR+MID 또는 MID+FULL만 보이는지 검사한다. 별채는 집 tier를 복제하지 않고 동일한 전역 물리 root가
    세 구간에 계속 보이며, overlay가 나타나면 두 base root와 집·담·mass·해당 별채 source range가 함께 접힌다.
...
- `check:giwa-tile-course`
  - 기와 기왓골이 세계좌표에서 등간격인지를 미터로 고정한다. 지붕면 UV의 `u`=`across/GIWA_ACROSS_PITCH`,
    `v`=`arcFromRidge/GIWA_ALONG_PITCH`, 수키와 롤 across 피치=암키와 피치, 회첨 튜브 하단=처마 코너,
    `sugiwaMaterial` TubeGeometry 축(repeat.x=length/along)을 함께 검사한다.
...
plan 기준값은 `tools/plan-contract.json`에 있다. 의도된 기능 변경도 실제 데이터 diff를 검토한 뒤 fixture를 갱신한다. 사찰 터만 바꾸는 변경에서는 temple ON 사례만 변할 수 있고, 모든 필지에 새 계획 필드를 추가하는 변경은 OFF/ON 전 사례가 함께 변할 수 있다.

### `npm run check:temple:browser`

`temple.html`을 독립 정적 서버에서 열어 `src/api/temple.js`의 세 변형을 실제 WebGL로 조립한다.

- 실제 렌더 bounds가 plan의 역할별 처마 polygon 및 마을이 예약한 경내와 2.5m 처마·담장 여유 안에 있는지 검사한다.
- 카메라가 남측 `frontDir` 접근 공간에서 26° 망원으로 보는지 검사한다.
- `disposeTempleCompound()`의 성공·멱등성, 호출자 palette 보존, 자체 palette와 geometry 해제를 검사한다.
- 실제 v1 plan 입력과 canonical v2가 같은 architecture·삼각형·draw/program/material 결과를 내고, 미래 schema가 WebGL 조립 경계에서도 거절되는지 검사한다.
- raw와 부감 merge의 삼각형·semantic count 동등성을 확인하고, merge가 draw call을 85% 이상 줄이며 모든 변형을 140콜 이하로 유지하는지 검사한다. 궁궐 전용 잡상·취두는 어느 사찰 변형에도 생기지 않아야 한다.
...
`check:drainage`는 공개 `src/api/drainage-plan.js`와 실제 `planVillage()` 통합 결과를 함께 검사한다.
같은 입력의 불변·JSON-safe record, 전역 RNG 비사용, 일반 농촌 `none`, capital 간선 한쪽,
Hanyang 계획가로 최대 양측 정책을 고정한다. 각 run은 저장된 실제 삼각 지형 `surfaceY`보다
12mm 이상 높고 2.5cm 상류 보정 안에서 하류로 단조이며 도로·다른 길·수계·필지·논 예약을
침범하지 않아야 한다.
대문 판석은 저장된 `access.gatePoint → roadPoint` 선분과 실제 도랑의 교점에만 생기고 보간한
도랑 lip보다 정해진 여유만 높아야 한다.

`shoot:drainage`는 고정 `capital:11` 실제 제품 마을에서 하나의 대문 건넘과 연결 도랑을 선택한다.
같은 카메라에서 layer OFF/ON 근경·부감 PNG를 찍고, 도랑+판석이 최대 두 mesh, 한 material family,
texture 0, shadow caster 0인지 확인한다. 두 번째 실제 마을의 plan JSON과 geometry hash가 같고
...
`tools/check-scene-snapshot.mjs`는 DOM·Svelte·Three·시계·전역 RNG 없이 versioned `scene` v1 코덱을 검사한다.
현재 seed와 이름 있는 시간대는 항상 고정하고, seed 기본값과 같은 환경·마을 값은 생략한다. 활성 마을은
연속 `siteR`, 지형/개울/강/논/수목 밀도, 성곽·시전, 집 수, 여섯 담장 가중치, 궁·절과 커밋된 주거 개구
...
기본값 생략 golden, 359.96°→0° 경계, 도달 가능한 OrbitControls 고도, 잘못된 계절/날씨·필드·타입,
미래 필드 fail-soft, encode→decode→encode byte 동일성, 최대 8개 편집과 1,900자 URL 예산을 함께 단언한다.
...
공유 결과에는 오직 하나의 `scene`만 남고, `hero`·`worker`·`shot`·`lang`·`flowsec`·`post` 같은 진입·검증
런타임 제어는 제거된다. Web Share 성공 시 clipboard를 호출하지 않고, 사용자가 공유 시트를 닫은
`AbortError`는 조용히 종료하며, 그 밖의 native 실패만 canonical URL 복사로 폴백한다. 두 경로가 모두
실패하면 명시적인 `failed` 결과를 반환한다.
...
- 실제 낙관을 눌러 고증 감사에 쓴 부엌·구들, 궁궐 지붕 장식, 창호 철물, 짚신·나막신 출처 묶음의 적용 문구와 원문 링크, 안전한 외부 링크 속성이 Reference UI에 렌더되는지 확인한다. AURI 주택 항목은 별채가 몸채·부속채·마당의 공간 어휘만 참고하고 확률·치수·clearance는 실측 역사 분포가 아닌 제품 정책임을 함께 표시한다. 창호 항목은 정적 민가·사찰 철물 어휘와 선택 FULL 주거만의 inward jamb-edge pivot 적용, 관아·궁·사찰 compound 제외, 열림 각도·감쇠가 실측 복원이 아닌 제품 결정임을 함께 보여야 한다. 신발 항목은 네 canonical URL과 라이선스 문구, 맑음/비 조건의 적용 범위를 실제 파싱 결과로 확인한다. #81의 AURI 실내측 한지 면과 국립민속박물관 작은 연소 광원은 각각 자기 Reference 항목 안에서 적용 제한·canonical URL·라이선스와 함께 파싱한다. 일부 어두운 방과 정확한 색/밝기/flicker/점등 비율이 제품 번역이라는 문구도 해당 항목을 벗어난 우연한 전역 문자열로 통과하지 못한다.
- References dialog는 제목에 초기 포커스를 두고 Tab/Shift+Tab을 양방향으로 가두며, 배경 app surface를
  `inert`/`aria-hidden`으로 격리한다. Escape·WASD·방향키는 선택 필지, walk, architectural reveal을 바꾸지
  않고 모달만 닫는다. 닫을 때는 정확한 brand/info opener, 반응형 재마운트된 같은 종류 trigger, 마지막으로
  안정적인 app surface 순으로 `inert` 해제 뒤 포커스를 복원한다. 390×844 sheet를 연 채 opener가 사라지는
  resize와 desktop 복귀 뒤 trigger 교체를 모두 같은 앱 부팅에서 검사한다.
- ContextPanel의 마을/집 본문과 sticky action은 같은 crossfade에 함께 보이는 동안에도 `morph=0.5` 경계의
  한쪽만 interactive owner다. 퇴장 subtree 안에 있던 포커스만 활성 브레드크럼 heading/trigger로 옮기고,
...
- 같은 앱 부팅에서 공유 버튼의 Enter 활성화와 실제 Web Share payload를 확인한다. 호출 순간의
  ...
  오차 안에서 유지되는지 검사한다. 별도 advanced fixture는 `siteR`·담장 가중치·지형/구성 옵션, stable focus,
  유효 편집 복원과 미래 stale parcel ID fail-soft, target-relative pan/광학 상태를
  최초 정착과 reload 뒤 모두 확인한다. 직접 확인이 필요하면
  `CHEOMA_APP_SMOKE_CAPTURE_DIR=/tmp/cheoma-share npm run check:app`으로 공유 UI 네 상태, snapshot focus
...
- 같은 최종 카메라 pose에서 움직임의 일반 표면 예산은 중심 1표본, hold/settle 뒤에는 대비가 큰 경계에서만 최대 13표본이며 focus·amount·aperture, pass 순서, composer/depth target/material identity와 크기, program cache-key·texture·geometry 수가 바뀌지 않는지 검사한다. HDR 광원 원판 scatter는 두 상태에서 같은 물리 크기와 에너지를 유지한다.
...
- 일곱 구도에서 앞집 지붕·산·패널에 선택 건물이 과도하게 가려지지 않는지 사람이 직접 확인한다.
...
- 사전 취소와 `terrain` 스텝 중 취소가 둘 다 `AbortError`로 종료되고, 두 rAF 후에도 스텝이 재개되지 않으며, 공유 Worker를 `terminate()`하지 않고 다음 일반 생성이 같은 Worker에서 성공하는지 확인한다.
...
`check:shader-warm`은 해제된 material을 건너뛰는 기존 계약에 더해 engine signal 취소 시 10ms polling timer가 재예약되지 않음을 확인한다. `check:app`의 기존 단일 부팅 teardown 단계는 in-flight Hanyang preload·GLB export를 실제로 시작한 뒤 unmount하고, export `AbortError`, late render/compile/cache 0, 소스 자원 정확히 1회 해제, dispose 후 공개 API 반환 표, 개별 debug hook identity와 캡처된 훅의 중립화를 함께 고정한다.
...
| `tools/check-walk-control.mjs` | 1인칭 **수동 조작** 계약(#33, 순수): 진입 기본이 정지(M1)이고 자동 산책은 명시 호출로만 켜진다(M11). 전진 명령의 이동 거리가 램프 포함 해석해와 ±1mm 로 일치하며(M2), 정지 명령 후 걷기 0.10s·달리기 0.16s 안에 속도가 **정확히 0** 이 되고 그 뒤 위치가 변하지 않는다(M3). 대각 입력 정규화(M4), 픽셀 감도(yaw 0.0026·pitch 0.0022 rad/px)와 피치 클램프 ±68.75°(M5·M6), 시선 델타의 **1프레임 1회 소비**(M7 — 드래그 한 번이 무한 회전이 되던 배선 결함의 회귀 게이트), 회전의 즉시 반영(M8), 경사 지형 눈높이 오차 <0.05m·침하 0(M9), 실제 village plan 위 120초 주행에서 충돌 0·경계 이탈 0·터널링 0(M10), `update(dt,input)` 하위 호환(M12). **M16 크리에이티브 비행**(2026-08-06 사용자 요청 "하늘도 날 수 있고"): 점프 한 번은 비행을 켜지 않고 판정 창(0.35s) 안의 두 번째 탭이 켠다, 비행 중 무입력이면 고도가 **정확히** 유지되며(중력 없음), 상승·하강이 저작값 7.5m/s, 천장 120m·바닥 눈높이 클램프, 다시 더블탭하면 낙하로 인수된다. **M17 발소리 케이던스**: 걸음은 시간이 아니라 **보폭 누적**에서 나오므로 케이던스가 속도에서 파생된다 — 걷기 2.65·달리기 3.13 걸음/s(보행 밴드 2.0~3.2), 비행 중 보행거리 적립 0(발이 닿지 않는다), 착지 충격은 한 번만 소비된다(착지음 중복 금지). R1 은 스텁 카메라·컨트롤로 `cinematic-runtime` 을 실제로 돌려 워킹뷰 진입 기본이 수동인지, 무입력 정지·명령 해제 정지가 성립하는지 본다. FAIL-first 확인(2026-08-01): M2·M3·M5·M7·R1 은 변경 전 HEAD 소스에서 실제로 실패, M1·M8 은 HEAD 에서도 통과(각각 코어 재발 방지·전방 보호막) | 렌더·DOM·키보드 이벤트 수집은 다루지 않는다(입력 수집은 `app/src/App.svelte` 소유, 브라우저 확인은 `check:cinematic:app`·수동 캡처). 발소리는 **소리 자체를 판정하지 않는다** — 합성 음색은 귀의 몫이고 여기서 보는 것은 케이던스 수치다. 배선(엔진 API → 시네마틱 런타임 → walker → 오디오)이 브라우저에서 실제로 도는지는 `scratch/fps-creative/probe.mjs` 가 오디오 노드 생성 수의 **구간 차분**으로 확인했다(걷기 3s 에서 증가, 비행 구간 증가 0, 콘솔 에러 0). 모바일에는 비행 토글 버튼이 없다(점프 버튼도 없다) — `engine.cine.setFly()` 는 열려 있으나 HUD 배선은 별도 라운드다. 자동 산책 경로의 품질(밀집 스팬·담장선 접근·스폰 밀도)은 `check-cine-quality.mjs` W1~W6 이 계속 소유한다. |
...
| `tools/check-cine-warm.mjs` | 드론 투어 셰이더 링크 히치(#36, 브라우저 — `check:cine-warm`, `check:cinematic:app`에 포함): 투어 시작 warm(`warmTourDetailShaders` — LOD 개방 compileAsync + 경로 표본 강제 draw + program settle) 이후 crane-in/street-flythrough 재생 전 구간에서 `debugProgramCount` **증가 0**을 village·hanyang 두 규모에서 단언한다. FAIL-first 확인(2026-08-02): warm 확장 전 소스에서 village crane-in +2(127→130 @t0.522)·hanyang street +2(141→143 @t0.004) 실측 실패 | frame ms는 판정하지 않는다(헤드리스 ANGLE은 링크 직렬화 — 프로그램 수 델타만 증거). warm 자체 비용(rAF 수)은 게이트 로그로만 보고한다. |
...
| `tools/verify-cine.mjs` | 세 규모 드론 투어 구간(4)과 walker 100초 수학·결정론 | 앱 배선은 확인하지 않는다. 투어 계약 본체는 `check-cine-quality.mjs` T1~T12 다. |
| `tools/check-cine-quality.mjs` | 감상 모드 순수 계약(#32 재저작 2026-08-01, 일곱 픽스처 = solo/hamlet/village/town/capital/hanyang + 종전 회귀 시드 village/2026). 드론뷰가 **컷 없는 연속 투어**로 재설계되어 특정 샷의 프레임 기하를 못박던 D1~D12 는 폐기하고 투어 불변식으로 옮겼다: T1 구간 이음매(위치·시선·fov)가 부동소수 오차 이내로 이어지고 마지막 구간의 끝이 첫 구간의 시작(루프 컷 0), T2 런타임과 동일한 재생(구간 경계에서 넘친 시간을 다음 구간으로 환산)에서 프레임 속도 2~34m/s·프레임당 변화 ≤3.6m/s, T3 시선 각속도 ≤45°/s·p99 ≤28°/s·프레임간 변화 ≤12°/s, T4 지형 클리어런스 >1.5m, T5 건물 관통 0·직하 지붕 위 ≥1.85m, T6 시선이 보호수 수관을 관통하지 않음, T7 앵글 어휘(저공 대역 ≥10%·street-flythrough 구간 저공 ≥28%·부감 최대 고도 ≥max(Hmax·0.6, R·0.18)·오빗 방위 커버 ≥200°·선회 본체 창에서 반경 상수), T8 피사체 커버리지(필지 ≥20 규모는 전체 ≥90%·구간 ≥75%, 그 이하는 65/45%), T9 오빗 중 랜드마크가 수평 화각 안 ≥95%, T10 구간 4종 이름·kind·duration 하한·투어 길이 60~205s·비유한 0, T11 결정론(같은 시드 동일·**다른 시드 상이**·sunAzimuth 미제공 결정론), T12 진입·리빌 역광 ≤32°·오빗 완전 역광이 구간 25~70% 지점. **2026-08-01 비전 FIX 추가**: T13 구간별 프레임 피치 median 대역(진입 -16~-26·정점 -34~-42, 선회 -14~-22, 저공 -12~-18, 리빌 -16~-26)·상단 빈 하늘 ≤24%·천저 고꾸라짐 금지(0~45°), T14 선회 중 주 랜드마크 AABB 8정점 투영의 하단 여백 ≥5%·세로 점유 ≥12% 를 표본 90% 이상에서 충족(하단 절단·얇은 수평 띠 금지), T15 프레임 하단 2/3 지면 레이캐스트에서 12m 안에 건축이 없는 표본 비율이 저공 구간 평균으로 밀집 도성(필지 ≥200) 45%·그 밖 68% 이하. 자동산책 W1~W6 은 단언 그대로 유지하되(2026-08-01 #33) 의미가 바뀌었다 — 워킹뷰 기본이 수동 조작이 되었으므로 이제 제품 기본 동작이 아니라 **명시 호출로만 켜지는 자동 산책 경로**(데모 클립·향후 유휴 드리프트)의 품질 계약이다. 수동 조작 계약은 `tools/check-walk-control.mjs` 소유 | 수관 볼륨·밀도를 plan 에서 독립 파생하므로 렌더 형상(gardens.js 수관 프로토)이 바뀌면 모델을 함께 갱신해야 한다. 마당 과실수는 plan 에 없어 다루지 않는다. `CHEOMA_CINE_CASES=village,capital` 로 픽스처를 걸러 빠르게 돌릴 수 있다(FAIL-first 확인용). 미감 판정은 하지 않는다(그건 `tools/shoot-cine.mjs` 프레임과 비전 에이전트가 맡는다). |
...
| `tools/check-sijeon-contract.mjs` | 기존 시전 위치 bytes, 순수 2칸 façade schema v2(소수 비발광 `signs[]` 포함), 시명/물종 필드 금지, 필지·도로 회랑 경계, 유한값·직렬화·결정론·무전역 RNG | Three 재질·병합 geometry와 실제 한양 화면은 `check:api-reuse`, app/worker 게이트가 맡는다. `shoot:sijeon`이 6 mesh/6 material(2026-08-08 재핀 — `9b5645f` façade v4 thatch 재질 추가)·texture 0·signCount를 기록한다. |
...
| `tools/check-rim-master.mjs` | #35-1 순수 계약: (1) 실제 `createFocusPolicyRuntime` 에 `setPostFocus(false)` 를 걸어 부감·시네마틱 정책이 `setRimEnabled(false)` 를 부르지 않고 `RIM_CONTEXT_MASTER.aerial > 0` 만 다이얼하는지(flare OFF·DoF 0 은 유지, focus 는 1.0 불변, dofAmount 단독 호출이 마스터를 흐트리지 않음), (2) `rimDistanceGate` 가 실측 부감 프레이밍(한양 crane-in 464m·pullback 432m·capital·village)에서 피사체 거리 페이드 ≥0.60 을 남기고 렌즈 밴드를 **확장만** 하는지(근경 60m·히어로 170m 무회귀, 비정상 입력은 렌즈 밴드로 폴백), (3) sunset 역광 처마 프래그먼트의 가산이 authored 피크의 ≥6% 이고 정오(altGate 0)에서는 정확히 0, 부감 수관(organic)이 건물 아래인지. post.js·engine.js 접합 2줄은 소스 계약으로 확인, (4) §6 처마 실이 sunset 블룸 문턱(0.80)의 125% 이상이고 보통 실루엣 면은 문턱 아래·기와 면은 0 이며 `uRimTileMul` 이 **에너지 캡 밖**에 곱해지는지(위치를 소스에서 단언 — 값만 보는 계약은 "캡 안에 갇혀 문턱의 32% 에서 막히던" 회귀를 놓친다) | GL 없이는 실제 `uRimScale` 값을 못 읽으므로 살아 있는 유니폼·프로그램 수는 `tools/shoot-rim-aerial.mjs` 가 확인한다. `CHEOMA_RIM_ROOT` 로 다른 트리(예: `git archive HEAD`)를 가리켜 FAIL-first 를 재현할 수 있다. |
...
| `tools/check-cloud-silhouette.mjs` | #53 S6 구름 실루엣 다양성: 위상 무결, 패밀리 비유사도(IoU·형상 거리), 미러 정체성, 씬 배치 위계, 방향 반복, wisp 외곽 꼬리, 드로우 예산 | 순수 노드(자체 esbuild 번들, 브라우저 없음). `npm run check:cloud-silhouette`. 실제 픽셀·림 합성은 `shoot:sky`·`check:rim`이 맡는다. |
| `tools/check-cloud-fade-residue.mjs` | #53 부감 페이드 HDR 림 잔존("펜선"): 페이드 계수가 opacity뿐 아니라 림·역광 투과 게인에도 걸리고, 하늘 ROI 잔여가 절대 강도·윤곽 구조 상한 아래이며 빌보드 바디는 남아 있는지 | 브라우저(capital 부감 sunset, 자체 서버·헤드리스). `npm run check:cloud-fade`. `CHEOMA_CLOUD_FADE_FAILFIRST=1`로 수정 전 림 잔존 재현. |
| `tools/shoot-rim-aerial.mjs` | 증거 수집기(판정 안 함): 제품 경로 부감·드론 프레임에서 `window.__rim` 의 살아 있는 `uRimScale`·강도·거리 밴드·카메라→피사체 거리·재질군 계수/지수·커버리지를 기록하고, 같은 포즈에서 마스터 0 ↔ 부감 ↔ focus 를 왕복하며 `renderer.info.programs` 델타 0 을 확인한다. 규모(village/capital/hanyang) × 시간대(sunset/정오) 별 OFF/ON 쌍과, **같은 포즈에서 유기물 계수·지수만 바꾼 A/B 변형**(`-organic070`·`-organic015`)을 남긴다 | 미감·수목 림 판정은 사람/비전 에이전트가 프레임을 보고 한다. **부감 프레이밍 트윈이 멎을 때까지 기다린 뒤에 캡처한다** — 기다리지 않으면 같은 규모라도 카메라→피사체 거리가 230~625m 로 흔들려(실측) 실행 간 픽셀 비교가 성립하지 않는다. 유기물 A/B 는 `window.__rim.setGroups`(검증 전용 uniform 쓰기, 프로그램 재컴파일 없음)로 한 포즈 안에서 변인을 통제한다. 한양 동기 생성이 느려 시나리오별 예외를 잡고 매 시나리오마다 MANIFEST 를 갱신한다(`CHEOMA_RIM_TIMEOUT_MS`·`CHEOMA_RIM_OUT`·`CHEOMA_RIM_WORKER`). |
| `tools/check-pixel-stats.mjs` | #38 순수 계약: 픽셀 통계 공용 라이브러리(`tools/lib/pixel-stats.mjs`) 자체를 코드로 합성한 픽스처로 검증한다. 인코드→디코드 왕복(다섯 스캔라인 필터 전부), 알려진 색의 HSV·선형휘도, 영역/비율 밴드 해석, hue 히스토그램과 rose/green 점유·peak60, 원형 hue 산포가 해석값 47.7°와 일치, 명도 3분위가 **선형 상대휘도**로 갈리는지(포화 파랑은 HSV value 최대이지만 암부다), 행 프로파일·평활 평면의 100행당 구배와 하단 하드 스텝, 차분 통계의 임계 경계 포함·채널 편향·최댓값 위치, 패치 표본·최근접 확대 크롭 | 임계는 갖지 않는다 — 판정 임계는 각 룩 게이트(`check-fog-wash`·`check-rim-master` 등)가 소유한다. 여기서 재는 것은 산술의 정확성뿐이다. FAIL-first 확인 완료(2026-08-01): 디코더 필터 4종·차분 임계 경계·3분위 정렬축·hue 산포 계수·peak60 창 폭·localStd·luma 가중·sRGB 디코드·패치 평균·크롭 스텝·영역 클램프·hue 감기·행 표본 간격 등 20건의 고의 변이를 모두 잡는다. |
| `tools/lib/pixel-stats.mjs` | (라이브러리, 게이트 아님) node 내장 zlib 만으로 PNG 를 읽고 통계를 낸다: `decodePng`/`encodePng`/`readPng`/`writePng`, `resolveRegion`·`band`, `rgbToHsv`·`srgbToLinear`·`relLum`·`luma709`, `hueHistogram`·`hueStats`·`terciles`, `luminanceField`·`localStd`·`rowProfile`·`smoothPlane`, `diffStats`, `patchSample`·`cropScaled` | 2026-08-01 룩 라운드가 스크래치로 세 벌씩 재제작하던 산술의 이관본이며, 각 함수 주석이 출처 스크립트를 적고 있다. 기존 게이트를 이 라이브러리로 갈아끼우는 작업은 별도 라운드다(현재 `tools/lib/png-metrics.mjs` 는 pngjs 기반으로 독립 존속). 캡처 PNG 를 락 없이 다시 분석할 수 있으므로, 브라우저 락을 잡기 전에 이미 있는 PNG 로 먼저 반증하라. |
| `tools/shoot-ab.mjs` | 같은 부팅 A/B 표준 하네스(판정 안 함). 한 부팅에서 포즈가 정착할 때까지 기다린 뒤 엔진을 정지시키고 A(제품 기준) → 변인 교체 → B 를 같은 프레임에서 캡처한다. 변인 교체는 (a) `hook` — 검증 전용 uniform 쓰기(예: `window.__rim.setMaster`), (b) `overlay` — 지정 소스 모듈을 다른 내용으로 서빙해 GLSL 자체를 바꾸는 두 방식. 산출물은 `A-*.png`·`B-*.png` + `MANIFEST.md`(카메라 수치·프로그램 수·적용 재질 수) + `diff.json`(pixel-stats 로 뽑은 전체·밴드별 차분과 색 통계) | **실행 간 비교는 이 도구의 계약이 아니다** — 부감 프레이밍 트윈이 멎기 전에는 카메라→피사체 거리가 230~625m 로 흔들려(2026-08-01 실측) 두 부팅의 PNG 는 변인이 아니라 포즈가 다른 그림이다. `overlay` 모드의 함정: three 는 `customProgramCacheKey` 가 같으면 캐시된 프로그램을 재사용해 `onBeforeCompile` 을 다시 부르지 않으므로 교체가 적용되지 않은 채 A/B 가 바이트 동일이 된다 — 변형 폼에 키 접미사를 붙여 재링크를 강제하고, 실제로 GLSL 이 바뀐 재질 수가 0 이면 던진다. 스펙의 치환 앵커가 소스에 없으면 부팅 전에 던진다. `--variant`(rim-master·rim-shader-off) `--scenario`(aerial·hero) `--out`, `CHEOMA_AB_OUT`·`CHEOMA_AB_TIMEOUT_MS`. |
