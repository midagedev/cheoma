...
- `check:citywall`
  ...
  - 대형 마을의 외곽 LOD 청크는 실제 필지 footprint로 원근을 분류하고, 앱과 같은 focus 카메라에서 가장 가까운 소유 필지가 복원 거리 안에 드는지 production seed로 검사.
  - 원경 impostor는 실제 variant 치수에서 초가 우진각과 기와 ㄱ자·좌우반전 지붕을 파생하고, 실제 팔레트의 선형색·이엉 노후색을 쓰는지 검사한다.
- `check:choga-roof`
