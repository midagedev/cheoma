...
npm run check:all           # check/app/petals/worker/audio/temple-browser 통합 게이트
...
영향 범위가 넓은 구조 변경은 반복 중 `check:pr`, 머지 전 `check:full`을 사용한다. `check:all`은 core/app/petals/worker/audio에 독립 사찰 WebGL 계약을 더한다.
