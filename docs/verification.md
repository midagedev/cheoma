...
- 같은 앱 부팅에서 공유 버튼의 Enter 활성화와 실제 Web Share payload를 확인한다. native 성공은 clipboard를
  건드리지 않고, `AbortError`는 토스트 없이 끝나며, native 실패→clipboard 성공과 두 경로 실패는 각각
  ...
  ContextPanel의 활성 house sticky footer 하나만 공유 액션을 소유한다. 직접 확인이 필요하면
  `CHEOMA_APP_SMOKE_CAPTURE_DIR=/tmp/cheoma-share npm run check:app`으로 이 세 상태의 PNG만 선택적으로 남긴다.
