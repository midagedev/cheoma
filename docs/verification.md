...
npm run shoot:focus-level   # 18° 주택·종가·관아 hero·궁·사찰 실제 앱 구도 6장
...
## #81 예정 게이트 — 실제 창호 야간 불빛

아래 이름은 #81 코드 PR에서 연결할 **계획**이며 이 문서 변경 시점에는 실행 가능한 package script나 통과
결과가 아니다. 구현 전에 명령만 추가하거나 수치가 측정됐다고 기록하지 않는다.

- `check:nightlights`의 순수 층은 초가와 기와 ㅡ·ㄱ·ㄷ의 변주·mirror·실제 fit·필지 변환마다 opening id,
  고정 한지 면 중심과 외향 법선이 production panel과 일치하는지 검사한다. plan은 JSON-safe·deep-frozen·
  seed 결정론적이어야 하고 전역 RNG를 소비하지 않는다.
- 같은 gate의 작은 WebGL depth fixture는 앞에서 보이는 불빛이 불투명 집·담·지형 뒤에서는 사라지고, 반대
  벽·지붕·하늘 위로 새지 않는지 실제 픽셀로 확인한다. `depthTest=true`, `depthWrite=false`, 기존 단일
  `THREE.Points` 유지와 draw·triangle·material·program·texture·render-target delta 0을 함께 기록한다.
- `check:worker`는 sync/Worker/fallback scene bytes와 seed를, `check:app`은 focus/rebuild/reroll/hop/wave/LOD
  수명 및 References UI를 맡는다. 근경 한지 emissive로 handoff할 때 이중 불빛·팝·잔상이 없어야 하며 프레임별
  traversal·raycast·heap allocation이 생기지 않아야 한다.
- `shoot:nightlights`는 고정 seed·카메라·밤 상태의 aerial/MID/focus PNG를 OS 임시 디렉터리에 만든다. 에이전트는
  실제 한지 면에 붙은 절제된 따뜻한 원형 bloom/bokeh, 어두운 방의 공존, 정상 가림, 자유 부유점 부재를 직접
  판정한다. 정확한 색·밝기·flicker·점등 비율은 사료값으로 승인하지 않는다.
...
- 실제 낙관을 눌러 고증 감사에 쓴 부엌·구들, 궁궐 지붕 장식, 창호 철물, 짚신·나막신 출처 묶음의 적용 문구와 원문 링크, 안전한 외부 링크 속성이 Reference UI에 렌더되는지 확인한다. 창호 항목은 정적 민가·사찰 철물 어휘와 선택 FULL 주거만의 inward jamb-edge pivot 적용, 관아·궁·사찰 compound 제외, 열림 각도·감쇠가 실측 복원이 아닌 제품 결정임을 함께 보여야 한다. 신발 항목은 네 canonical URL과 라이선스 문구, 맑음/비 조건의 적용 범위를 실제 파싱 결과로 확인한다. #81 구현 때는 AURI의 실제 실내측 한지 면 설명과 국립민속박물관의 작은 연소 점광원 자료가 별도 항목으로 보이고, 일부 어두운 방·정확한 색/밝기/flicker/점등 비율은 제품 번역이라는 제한과 두 canonical URL이 실제 파싱 결과에 포함되는지도 추가한다.
...
- 정확한 18° 카메라 고도에서 1.65–2.5m 문 높이 target을 바라보고 별도 하늘 편향 없이 전경 마당을 화면에 남기는지 검사한다.
...
- 일반 주택과 종가의 화면 중심은 지붕 높이가 아닌 1.65–2.5m 문 높이 범위를 조준하고, 안전 가시성 dolly 뒤에도 카메라는 그 target을 향한 정확한 18° 고도를 유지한다.
...
| `tools/check-dof-app.mjs` | 실제 광각↔망원 focus, 조준점→문·문→문·focus-out 의미 초점 수명, rebuild/reroll 갱신·복합체 fallback과 StableBokeh 깊이 제외·상태 복원 | Bokeh 실제 렌더는 한 프레임만 수행하며 미감 판정을 대신하지 않는다. |
...
| `tools/check-rim-facing.mjs` | 실제 WebGL에서 N·V 실루엣, 실제 태양 N·L, V·L 역광, directDiffuse=0·그림자·제품형 비그림자 fill·`receiveShadow=false` 반례, 50% penumbra 비율, 실제 cloud-shadow callback/cache-key 합성, 추가 shadow fetch 0, HDR 에너지 상한, instanceColor·program plateau; `--app`은 시간을 멈춘 최종 18° 석양 focus에서 실제 마을 cloud+rim 지붕과 OFF/ON A/B | 고정 fixture와 대표 실앱 한 장이 모든 seed·재질의 미학을 대신하지 않는다. |
...
| 실제 창호 야간 불빛 (#81, 예정) | [`exterior-detail.md`](exterior-detail.md) §4.2 + 계획된 `check:nightlights` 순수 anchor/depth fixture + `check:worker` + `check:app` 수명·References UI + 계획된 `shoot:nightlights` 고정 aerial/MID/focus PNG + 기존 단일 Points GPU 0-delta |
