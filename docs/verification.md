...
`tools/check-share.mjs`는 브라우저·Svelte·Three 없이 현재 장면 링크의 순수 계약을 검사한다. 공유 URL은
현재 `seed`와 `time`을 항상 고정하고, 명시적으로 바뀐 환경·렌더 모드와 활성 마을의 `vseed`·규모·궁·절,
시간 흐르기, 커밋된 주거 개구 편집(`vedit`)을 canonical 순서로 보존한다. 반면 `hero`·`worker`·`shot`·
`lang`·`flowsec` 같은 진입·검증 런타임 제어는 제거한다. 플랫폼 어댑터는 Web Share 성공 시 clipboard를
호출하지 않고, 사용자가 공유 시트를 닫은 `AbortError`는 조용히 종료하며, 그 밖의 native 실패만 URL 복사로
폴백한다. 두 경로가 모두 실패하면 명시적인 `failed` 결과를 반환한다.
...
- 같은 앱 부팅에서 공유 버튼의 Enter 활성화와 실제 Web Share payload를 확인한다. 호출 순간의
  ...
  payload에서 이전 마을 필드가 사라지는지도 확인한다. 직접 확인이 필요하면
  `CHEOMA_APP_SMOKE_CAPTURE_DIR=/tmp/cheoma-share npm run check:app`으로 이 네 상태의 PNG만 선택적으로 남긴다.
