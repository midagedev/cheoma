...
- `check:lod`
  - 부감 46° 광각, 필지 10°·hero 7°·궁 24°·사찰 26° 망원과 보상 dolly가 피사체 화면 크기를 보존하고, 생활·입자 LOD 및 point sprite 크기를 바꾸지 않는지 검사. Points 계열은 가장 좁은 authored lens의 전체 scale을 받은 뒤 shader별 pixel cap을 지킨다. `check:petals`는 production snow shader의 고정 한 점을 21°/42m와 7°/127m에서 실제 WebGL로 그려 픽셀 지름과 coverage가 같은지도 비교한다.
