...
npm run check:lod:app       # 실제 한양 focus 전환의 프레임별 LOD·동물 sleep 계약
...
npm run check:layout        # 좌향·집 fit·도로/개울/논·일조/보호수·밀도 계약
...
npm run check:all           # check/app/petals/worker/audio 통합 게이트
npm run shoot:dof           # 고정 seed·정지 프레임 DoF 미감 비교(PNG는 OS 임시 폴더)
npm run shoot:bokeh         # 제어 광원 원형비·정지 동일성·저속 패닝 보케 fixture
npm run shoot:wave          # 실제 앱의 먹안개 handoff·그림자·program plateau 촬영
```
...
- `check:layout`
  ...
  - 개별 seed와 scale 합계의 호수 하한으로 과도한 빈집 회귀를 막는다. 남측 `solarAccess`, 보호수 실제 수관, 논, 개울, 숲/근경 식생 index가 같은 radius-aware footprint 계약을 쓰는지, 논끼리 겹침이 없고 종자·규모별 최소 배미 수를 지키는지도 검사한다.
- `check:wall-gate`
...
- `check:yard`
  ...
  - Issue #17에서 확인된 `hamlet:7:p0` 장독대와 `hanyang:7:p254` 부속채 관통점을 고정 회귀로 두고, 안전한 다른 슬롯으로 이동한 뒤에도 나무 수와 반복 결정론이 유지되는지 확인한다.
- `check:lod`
...
| `tools/check-layout-contract.mjs` | 남향 군집·도로측 대문·실제 지붕 fit·단건 재굴림·도로/개울/논·일조/보호수·밀도 계약 | 대표 seed 순수 데이터 검사로, 처마와 담의 실제 실루엣은 보지 않는다. |
| `tools/check-wall-gate-contract.mjs` | 6종 담과 hero의 도로측 대문 중심·회전·finite geometry | Node에서 실제 담 생성기를 bundle하며, 완성 화면의 미감은 보지 않는다. |
| `tools/check-yard-layout-contract.mjs` | 마당나무와 부속채·장독대·낟가리·빨래줄·텃밭·정원석의 의미별 footprint, p0/p254 재현, 수목 유지율 | 실제 수관/밑동의 XZ 계약이며 계절별 가지 실루엣의 미감은 focus 이미지로 본다. |
...
| `tools/check-lod-app.mjs` | 실제 Hanyang focus/hop/out, 동물·낙엽 휴면, scale wave/cancel 자원 수명 | SwiftShader shader link가 느려 독립 실행하며 `check:all`에는 넣지 않는다. |
...
| `tools/shoot-wave.mjs` | 실제 Vite 앱의 규모별 고정 progress 전환, 재질/version/cache key 불변, 그림자·program plateau | old/new seed·시간·계절·날씨·구간은 `CHEOMA_WAVE_*`로 고정하며 PNG는 OS 임시 폴더에 쓴다. |
...
| shader/material/post/roof | 위 게이트 + `check:dof:app`(DoF 관련) + program count와 고정 시드 이미지 직접 판정 |
