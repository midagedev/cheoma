...
루트 `package.json`은 Playwright 도구용이고 `app/package.json`은 SPA용이다.
...
npm run check:all           # 위 세 게이트 순차 실행
```
...
에이전트는 생성 PNG를 직접 열어 관통, 실루엣, 숲 프레이밍, 길의 연속성을 본다. 수치 통과는 미학적 통과를 대신하지 않는다.

Headless ANGLE은 shader link를 직렬화하므로 절대 frame time을 실제 성능 수치로 인용하지 않는다. program 수 변화, draw call, 결정론 hash, 오류 수를 우선한다. 임시 이미지는 scratch 경로에 두고 기존 `shots/` 증거를 덮어쓰지 않는다.
