...
PR을 머지하기 직전 최종 변경이 끝난 동일 HEAD에서 `npm run check:full` 성공 1회를 남긴다. 이후 diff가 바뀌면 다시 실행한다. 이는 전체 browser-free 계약과 app/particle geometry/Worker/audio/
...
npm run check:all             # core/app/particle/Worker/audio/temple/parcel/surface profile
...
영향 범위가 넓은 구조 변경은 반복 중 `check:pr`, 머지 전 `check:full`을 사용한다. `check:all`은 core/app/petals/particle-geometry/worker/audio에 독립 사찰 WebGL과 단건 재건축 앱 계약을 더한다.
...
- `npm run check:particle-geometry`는 강수, 꽃잎·모트, 한지 불빛을 하나의 체크포인트로 묶는다. 강수와 한지
  자원·결정론 계약은 browser-free fast 목록에도 포함하고, 꽃잎·모트의 실제 world geometry·가림·DoF depth는
  shared browser lock 아래의 작은 WebGL fixture 하나로 검사한다. `check:pr`는 이 표현들의 생성기, 물리 geometry,
  계절 선택 및 공개 particle API가 바뀌면 체크포인트를 자동 선택한다. 화면 미감은 별도의 폐기된 A/B 촬영 도구가
  아니라 영향받은 제품 app/DoF 게이트에서 확인한다.
...
| 물리 강수·꽃잎/모트·실제 창호 야간 불빛 (#81/#96) | [`exterior-detail.md`](exterior-detail.md) §4.2 + `check:particle-geometry`의 world geometry·결정론·가림·명시적 DoF depth + `check:nightlights`의 실제 anchor·owner 수명 + `check:worker` scene bytes + `check:parcel-rebuild:browser` owner 복원 + `check:app` 항목별 References UI |
