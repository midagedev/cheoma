...
| `tools/check-cine-quality.mjs` | 감상 모드 **화면 품질** 순수 계약(#30, 네 픽스처 = village/capital/hanyang + verify-cine 의 village 시드): flythrough 가 시가지 밀도 0 구간을 통과하지 않고(서브미터 필지 틈 1% 허용), 고도가 지형이 아니라 민가 지붕선을 추종하며(고정 18m 원판·민가만, 잔차 표준편차 비교), look.y −0.20~−0.15 로 지평선이 프레임에 들어오고, 밀집면 중심이 프레임 중앙에서 median ≤15°·p90 ≤45° 안에 있고, 궁이 있는 규모는 패스 종점에서 랜드마크가 수평 화각 안에 들어오며, 패스 종반(u≥0.7)의 **시선 원뿔** 안에 건축이 담기고(20 표본 중 min ≥3·median ≥6 — 위치 밀도만 보면 "옆에는 집이 있는데 프레임은 비어 있는" 종반을 놓친다), orbit 시선이 보호수 수관을 관통하지 않고 선회 반경은 상수, crane-in 하향 22~26°, pullback 최종 컷 하늘 35~40%, `sunAzimuth` 제공 시 태양-시선 이각 ≤25°·orbit 완전 역광이 스윕 35~45% 지점, 모든 패스 시선 각속도(orbit ≤30°/s, 그 외 ≤60°/s), 자동산책 pitch −6~−9°·회전 ≤26°/s·스폰이 밀도 심부·담장선 1.5~2m 접근·하단 프레임 대역 담 점유율 ≥60%·200s 이동거리 | 수관 볼륨과 밀도를 plan 에서 독립 파생하므로 렌더 형상(gardens.js 수관 프로토)이 바뀌면 모델을 함께 갱신해야 한다. 마당 과실수는 plan 에 없어 다루지 않는다. 18m 안에 민가가 없는 가로(hanyang 육조거리)는 지붕선 추종 판정을 건너뛴다. 미감 판정은 하지 않는다(그건 `tools/shoot-cine.mjs` 프레임과 비전 에이전트가 맡는다). |
