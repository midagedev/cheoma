...
거리 feedback으로 풀고, report에 root owner/weight와 calls/triangles/programs를 남긴다. 에이전트는 중간
...
| `tools/check-lod-app.mjs` | `focus/wave/full` 선택 실행: Hanyang screen-door channel/자원·focus/hop/out과 town→village fast wave 또는 Hanyang 전체 연속 흐름 | 빠른 wave가 Hanyang multi-chunk full을 대체하지 않아 merge 전 `check:full`을 유지한다. |
