...
const PR36_TREE_BASELINES = {
  ...
  //
  // #23 R5b 관아 열(2026-08-02) 재기준: 한양 3케이스만 이동한다. 육조 관아 슬롯이 절대 오프셋
  //   (26·54m)에서 축선 span 종속(이격 링~종로 균등 분할)으로 바뀌면서 예약 blocker 가 옮겨졌고,
  //   그 하류로 필지 재배치가 일어나 마당나무를 가진 필지도 함께 움직였다.
  //   단독 변인 증거(실측 2026-08-02, `git show HEAD:src/village/palace-precinct-plan.js` 로
  //   되돌린 소스 vs 이 라운드 소스, 같은 스크립트 `scratch/r5b/measure-flora.mjs`):
  //     hamlet   5/5/2      → 5/5/2      (불변)
  //     village  20/15/11   → 20/15/11   (불변)
  //     town     62/56/44   → 62/56/44   (불변)
  //     capital  3/4/5      → 3/4/5      (불변)
  //     hanyang  81/62/53   → 87/59/46
  //   궁이 있어도 관아 슬롯이 없는 capital(magistracy.max 0)까지 불변인 것이 변인이 관아 열
  //   하나임을 못박는다. 서편 채움(FIX-A)은 palace.js 렌더러 전용이라 buildVillageFlora 에
  //   들어가지 않는다.
  //   **완화가 아니라 재기준이다**: 유지율 하한(케이스 0.80 · 전체 0.94)과 모든 교차 단언은
  //   그대로다. 종전 값 91/61/64 는 이 라운드 **이전에도** 재현되지 않았다(HEAD 실측 81/62/53)
  //   — 기준선이 R2 시점에 고정된 채 뒤따르는 라운드들이 0.80 여유 안에서 움직였기 때문이고,
  //   여기서 실측으로 다시 못박는다.
  'hanyang:7': 87, 'hanyang:42': 59, 'hanyang:20260716': 46,
};
