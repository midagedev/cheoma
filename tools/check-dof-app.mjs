...
try {
  ...
  const result = await page.evaluate(async (threeModuleUrl) => {
    const THREE = await import(threeModuleUrl);
    const engine = window.__engine;
    engine.setViewShiftEnabled(false);
    let morph = 0;
    const offMorph = engine.on("villageFocusMorph", (value) => {
      morph = value;
    });
    const snapshot = () => ({
      ...engine.debugDof(),
    ...
    const doorFocusSnapshot = (parcelId) => {
      ...
      const delta =
        Array.isArray(anchor) && Array.isArray(center)
    ...
    const begin = async (action) => {
      ...
      if (engine.debugDof().tweenProgress == null)
        throw new Error("DoF transition did not start");
    };
    const seek = (points = [0, 0.25, 0.5, 0.75, 1]) =>
      points.map((progress, index) => {
        const sample = engine.debugDofSeek(progress, {
          finish: index === points.length - 1 && progress === 1,
        });
        if (!sample)
          throw new Error(`DoF transition vanished at progress ${progress}`);
        return { ...sample, morph };
      });

    const fixtures = engine.village
      .debugParcels()
      .map((parcel) => parcel.parcelId);
    const required = ["p32", "p27", "p16"];
    if (!required.every((id) => fixtures.includes(id))) {
      throw new Error(
        `missing deterministic DoF fixtures: ${required.filter((id) => !fixtures.includes(id)).join(", ")}`,
      );
    }
    ...
    // depth material, then restore the authored time level below.
    nightLightsApi?.setLevel(1);
    const visibleSentinels = [];
    const materialSentinels = [];
    const sourceDepthSentinels = [];
        ...
        (object.isPoints ||
          ...
          (object.isMesh &&
            (Array.isArray(object.material)
              ...
              .filter(Boolean)
              .every((material) => material.depthWrite === false)))
      ) {
    ...
    const sunGlow = engine.scene.getObjectByName("sunGlow");
    const sunGlowContract = {
      exists: !!sunGlow?.isSprite,
      explicitDepth: !!sunGlow?.userData?.dofDepthMaterial,
      includedInSourceDepth: sourceDepthSentinels.some(
        ([object]) => object === sunGlow,
      ),
      additive: sunGlow?.material?.blending === THREE.AdditiveBlending,
      transparent: sunGlow?.material?.transparent === true,
      skipsDepthWrite: sunGlow?.material?.depthWrite === false,
    };
    const overrideBefore = engine.scene.overrideMaterial;
    const backgroundBefore = engine.scene.background;
    const initialRecovery = [];
    for (let i = 0; i < 30; i++)
      initialRecovery.push(engine.debugAdvancePostQuality(1 / 60));
    const beforeDepthFrame = {
      dof: snapshot(),
      visibleCount: visibleSentinels.length,
      materialCount: materialSentinels.length,
      sourceDepthCount: sourceDepthSentinels.length,
      sunGlowContract,
      fadedTreesKeepRim: materialSentinels.every(([, material]) =>
        (Array.isArray(material) ? material : [material])
          .filter(Boolean)
          .every((part) => part.userData?.__rimPatched),
      ),
    };
    ...
    const settleQuality = [];
    for (let i = 0; i < 30; i++)
      settleQuality.push(engine.debugAdvancePostQuality(1 / 60));
    engine.debugRenderDofFrame();
    ...
    const afterDepthFrame = {
      ...
      materialsRestored: materialSentinels.every(
        ([object, material]) => object.material === material,
      ),
      sourceDepthMaterialsRestored: sourceDepthSentinels.every(
    ...
    nightLightsApi?.setLevel(nightLightsLevelBefore);
    const postQuality = {
      ...
      resourcesStable:
        resourceKeys.every(
          (key) => resourcesBefore[key] === resourcesAfter[key],
        ) &&
        resourcesBefore.passCount === resourcesAfter.passCount &&
        resourcesAfter.bokehMaterial.version === materialVersionBefore,
      packedDepthReused:
        resourcesBefore.bokehMaterial.uniforms.tDepth.value ===
          resourcesBefore.depthTexture &&
        resourcesAfter.bokehMaterial.uniforms.tDepth.value ===
          resourcesAfter.depthTexture,
      resolutionStable:
        JSON.stringify(resolutionBefore) === JSON.stringify(resolutionAfter),
      passOrderStable:
        JSON.stringify(passOrderBefore) === JSON.stringify(passOrderAfter),
      programCountBefore,
      programCountAfter: engine.renderer.info.programs?.length ?? null,
      programKeysStable:
        JSON.stringify(programKeysBefore) ===
        JSON.stringify(
      ...
      depthSizeStable:
        resourcesBefore.depthTarget.width ===
          resourcesAfter.depthTarget.width &&
        resourcesBefore.depthTarget.height ===
          resourcesAfter.depthTarget.height,
      memoryBefore,
      memoryAfter: { ...engine.renderer.info.memory },
      depthParity:
        movingDepth.depthExcluded === stableQuality.depthExcluded &&
        ...
        movingDepth.sourceDepthMaterials === stableQuality.sourceDepthMaterials,
    };
    ...
    const exitHouse = [0, 0.25, 0.5, 0.75, 0.999, 1].map(
      (progress, index, points) => {
        ...
        return {
          ...dof,
          optics,
          hasReferenceFov: Object.hasOwn(
            engine.camera.userData,
            "villageReferenceFov",
          ),
    ...
    return {
      ...
      reverse: {
        in: reverseIn,
        out: reverseOut,
        startAmount: reverseStartAmount,
        end: reverseEnd,
      },
      exitHouse,
      plainHouse,
  ...
  const maxError = (samples) =>
    Math.max(0, ...active(samples).map((sample) => sample.error ?? Infinity));
  const apertureInRange = (samples) =>
    samples.every(
      (sample) =>
        sample.aperture >= -1e-12 &&
        sample.aperture <= sample.baseAperture + 1e-12,
    );
  const monotonic = (values, direction) =>
    values.every(
      (value, index) =>
        index === 0 ||
        (direction > 0
          ? value >= values[index - 1] - 1e-6
          : value <= values[index - 1] + 1e-6),
    );
  ...
  const finiteAnchor = (sample) =>
    Array.isArray(sample?.anchorWorld) &&
  ...
  pass(
    result.aerial.enabled === false &&
      result.aerial.amount === 0 &&
      result.aerial.aperture === 0,
  ...
  pass(
    result.focusEnd.continuum.aerialReferenceDist > 0 &&
      result.focusEnd.continuum.focusMaxReferenceDist >=
        result.focusEnd.continuum.aerialReferenceDist &&
      Math.abs(
        result.focusEnd.zoomMax - result.focusEnd.continuum.focusMaxActualDist,
      ) < 0.2,
  ...
  pass(
    monotonic(result.postQuality.settle, 1) &&
      result.postQuality.settle.every(
        (quality) => quality >= 0 && quality <= 1,
      ),
    "post quality hold and settling recovery are monotonic without overshoot",
  );
  pass(
    result.postQuality.policyMoving.amount ===
      result.postQuality.policyStable.amount &&
      Math.abs(
        result.postQuality.policyMoving.focus -
          result.postQuality.policyStable.focus,
      ) < 1e-9 &&
      Math.abs(
        result.postQuality.policyMoving.aperture -
          result.postQuality.policyStable.aperture,
      ) < 1e-12,
    "adaptive quality leaves DoF amount, focus, and aperture unchanged",
  );
  pass(
    result.postQuality.resourcesStable &&
      result.postQuality.packedDepthReused &&
      ...
      result.postQuality.programCountAfter ===
        result.postQuality.programCountBefore &&
      result.postQuality.memoryAfter.textures ===
  ...
  const depthJitter = Math.abs(
    result.depthFrame.after.dof.focus - result.depthFrame.before.dof.focus,
  );
  pass(
    result.depthFrame.after.dof.depthExcluded > 0,
    `Bokeh depth excludes decorative/non-depth objects (${result.depthFrame.after.dof.depthExcluded})`,
  );
  pass(
    result.depthFrame.before.visibleCount > 0 &&
      result.depthFrame.after.visibilityRestored,
    `depth prepass restores all decorative visibility (${result.depthFrame.before.visibleCount} sentinels)`,
  );
  pass(
    result.depthFrame.before.materialCount > 0 &&
      result.depthFrame.after.dof.depthDithered ===
        result.depthFrame.before.materialCount &&
  ...
  pass(
    result.depthFrame.before.sourceDepthCount > 0 &&
      result.depthFrame.after.dof.sourceDepthMaterials ===
        result.depthFrame.before.sourceDepthCount &&
      result.depthFrame.after.sourceDepthMaterialsRestored,
    `depth prepass uses and restores explicit source-depth materials (${
      result.depthFrame.before.sourceDepthCount
    } sentinels)`,
  );
  pass(
    result.depthFrame.before.sunGlowContract.exists &&
      !result.depthFrame.before.sunGlowContract.explicitDepth &&
      !result.depthFrame.before.sunGlowContract.includedInSourceDepth &&
      result.depthFrame.before.sunGlowContract.additive &&
      result.depthFrame.before.sunGlowContract.transparent &&
      result.depthFrame.before.sunGlowContract.skipsDepthWrite,
    "broad additive sun glow stays outside the compact-source depth contract",
  );
  pass(
    result.depthFrame.before.fadedTreesKeepRim,
    "instFade tree materials retain the flagship Fresnel rim patch",
  );
  pass(
    result.depthFrame.after.overrideRestored,
    "depth prepass restores the pre-existing scene override material",
  );
  pass(
    result.depthFrame.after.backgroundRestored,
    "depth prepass restores the scene background after preserving far-depth sky",
  );
  pass(
    depthJitter < 0.01 && result.depthFrame.after.dof.amount === 1,
  ...
  pass(
    result.rebuild.rebuilt &&
      ...
      result.rebuild.after.dof.semanticWrites >
        result.rebuild.before.dof.semanticWrites &&
  ...
  pass(
    result.reroll.seedBefore !== result.reroll.seedAfter &&
      result.reroll.revealKind === "rebuild" &&
      result.reroll.after.dof.semanticParcel === "p32" &&
      result.reroll.after.dof.semanticWrites ===
        result.reroll.before.dof.semanticWrites + 1 &&
      result.reroll.after.dof.anchorSource === "primary-opening" &&
      finiteAnchor(result.reroll.after.dof) &&
      primaryDoorAligned(result.reroll.restored, [
        "primary-opening-transition",
      ]),
    `focused reroll atomically replaces the semantic anchor (seed ${
      result.reroll.seedBefore
    }->${result.reroll.seedAfter}, writes ${
      result.reroll.before.dof.semanticWrites
    }->${result.reroll.after.dof.semanticWrites}, source ${
      result.reroll.after.dof.anchorSource
    }, reveal ${result.reroll.revealKind}, rest-plane ${
      result.reroll.restored.portalPlaneOffset
    })`,
  );

  pass(
    maxError(result.returnOuter) < 0.01 &&
      monotonic(
        result.returnOuter.map((sample) => sample.amount),
        -1,
      ) &&
      result.returnEnd.enabled === false &&
      result.returnEnd.transitioning === false,
    "focus-out holds the departing house depth while DoF fades monotonically to off",
  );
  pass(
    result.returnOuter.every(
      (sample) =>
        sample.anchorSource === "primary-opening-transition" &&
        finiteAnchor(sample),
    ) &&
  ...
  pass(
    finite(result.focusHopStart) &&
      finite(result.hop) &&
      result.hopEnd.selected === "p16" &&
      result.hopEnd.transitioning === false,
    "long house hop completes with finite DoF state",
  );
  pass(
    primaryDoorAligned(result.focusHopFromDoor) &&
  ...
  pass(
    maxError(result.hop) < 0.01 &&
      result.hop.every(
        (sample) => sample.enabled && Math.abs(sample.amount - 1) < 1e-9,
      ) &&
  ...
  pass(
    result.returnHop.at(-1).finished === true,
    "hop fixture returns cleanly to aerial",
  );

  pass(
    result.reverse.startAmount > 0 && result.reverse.startAmount < 1,
    `mid-focus reversal starts from its current amount (${result.reverse.startAmount.toFixed(3)})`,
  );
  pass(
    apertureInRange(result.reverse.out) &&
      monotonic(
        result.reverse.out.map((sample) => sample.amount),
        -1,
      ) &&
      maxError(result.reverse.out) < 0.01 &&
      result.reverse.end.amount === 0 &&
      result.reverse.end.enabled === false,
  ...
  const opticalContinuity = result.exitHouse.every((sample) => {
    ...
    return (
      [physicalDistance, visualDistance, lensScale, sample.moteLensScale].every(
        Number.isFinite,
      ) &&
  ...
  pass(
    result.plainHouse.every(
      (sample) =>
        !sample.hasReferenceFov && Math.abs(sample.optics.lensScale - 1) < 1e-9,
    ),
    "position-only house tweens do not leak village lens metadata",
  );
  pass(
    runtimeErrors.length === 0,
    `browser reports no runtime errors (${runtimeErrors.length})`,
  );
  ...
  if (runtimeErrors.length)
    console.error(runtimeErrors.slice(0, 10).join("\n"));
} finally {
  await browser?.close();
  await server.close();
