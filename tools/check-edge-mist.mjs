...
// 부감은 링을 완전히 끄지 않는다(R5/U1 #211). 지형 절단면·외곽 수관을 대기로 녹이는 유일한
// 장치라 0 이면 마을이 하늘에 하드컷으로 붙는다. 웨지가 두드러지지 않는 바닥 가중치까지만 내린다.
assert.ok(EDGE_MIST_AERIAL_FLOOR > 0 && EDGE_MIST_AERIAL_FLOOR < 1,
  'the aerial floor must be a bounded partial weight');
// #211: capital/hanyang 부감에서 scene fog 가 절단면까지 못 미치므로 바닥값은 충분히 높아야 한다.
assert.ok(EDGE_MIST_AERIAL_FLOOR >= 0.8,
  'aerial edge-mist floor must stay high enough to dissolve the terrain cut');
...
// Fade ends at 30°, so 25° is still in the tail of the band — require a high residual, not
// exact floor. At ≥30° the floor is exact so the basin rim never fully vanishes mid-orbit.
const capitalAerialEdge = edgeMistViewWeight(forwardY(25));
assert.ok(capitalAerialEdge >= EDGE_MIST_AERIAL_FLOOR && capitalAerialEdge > 0.85,
  `product capital aerial pitch must keep a high edge-mist residual (${capitalAerialEdge})`);
...
assert.ok(RIDGE_MIST_AERIAL_FLOOR >= 0.5,
