...
const nextS = (s) => SEASONS[(SEASONS.indexOf(s) + 1) % SEASONS.length];
...
console.log('\n=== ② 눈 정리(겨울 밖) + 비 유지 ===');
{
  // 초기 winter+snow 는 랩(winter→spring)에서 눈→맑음 정리되는 일회성 이벤트 → 이 이벤트를 실제로
  // 관측하려면 관측 시작 시점에 아직 초기 winter 이어야 한다. flowsec=6 은 로드 중(렌더된 시간 기준)
  // 첫 전진 전에 관측이 시작되게 하는 하한(실측: obs-start=night/winter/snow 보존). 랩 전후 상태로 판정.
  const { page, context } = await open('hero=0&flow=1&flowsec=6&time=night&season=winter&weather=snow&lang=en&seed=7');
  const seq = await envSeq(page, 18000);
  const w = seq.findIndex((s, i) => i > 0 && seq[i - 1].season === 'winter' && s.season === 'spring');
  const pre = w > 0 ? seq[w - 1] : null, post = w > 0 ? seq[w] : null;
  check('겨울→봄 전진', !!post && post.season === 'spring', post ? post.season : 'no-wrap');
  check('겨울 밖으로 나가며 눈→맑음 정리', !!pre && pre.weather === 'snow' && post.weather === 'clear', pre ? `${pre.weather}→${post.weather}` : 'no-wrap');
  check('URL weather=clear 반영', (await search(page)).includes('weather=clear'));
  await context.close();
}
{
  const { page, context } = await open('hero=0&flow=1&flowsec=6&time=night&season=summer&weather=rain&lang=en&seed=7');
  const seq = await envSeq(page, 18000);
  const w = seq.findIndex((s, i) => i > 0 && seq[i - 1].season === 'summer' && s.season === 'autumn');
  const post = w > 0 ? seq[w] : null;
  check('비는 호환 계절 전진에도 유지(사용자 설정 존중)', !!post && post.season === 'autumn' && post.weather === 'rain', post ? `${post.season}/${post.weather}` : 'no-wrap');
  await context.close();
}
