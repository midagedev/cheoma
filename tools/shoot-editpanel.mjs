...
async function focusParcel(page, id, panelSel = '[data-make-panel].open') {
...
async function expandAdvanced(page, scope = '[data-make-panel]') {
...
const aerialHasSlider = await ev(desk, () => !!document.querySelector('[data-make-panel] input.scale'));
const aerialHasChar = await ev(desk, () => [...document.querySelectorAll('[data-make-panel] .opt, [data-make-panel] h4')].some((n) => /성격|character|민촌|여염|반촌/i.test(n.textContent || '')));
...
const sel = '[data-make-panel] input[data-key="footprintScale"]';
...
if (hanok) {
  ...
  await ev(desk, () => {
    const el = document.querySelector('[data-make-panel] input[data-key="riseScale"]');
...
if (palace) {
  ...
  await ev(desk, () => {
    const eave = document.querySelector('[data-make-panel] input[data-key="eaveOverhang"]');
    if (eave) { eave.value = '2.15'; eave.dispatchEvent(new Event('input', { bubbles: true })); eave.dispatchEvent(new Event('change', { bubbles: true })); }
    const bs = document.querySelector('[data-make-panel] input[data-key="bracketScale"]');
  ...
  await expandAdvanced(desk);
  await ev(desk, () => {
    const rows = [...document.querySelectorAll('[data-make-panel] section')].flatMap((s) => [...s.querySelectorAll('.row.bays')]);
    ...
    const tgls = [...document.querySelectorAll('[data-make-panel] .tgl')];
...
await ev(mob, () => { const grip = document.querySelector('[data-make-panel] .grip'); if (grip) grip.click(); });
await ev(mob, () => new Promise((r) => setTimeout(r, 500)));
await ev(mob, () => { const b = document.querySelector('[data-make-panel] .advtoggle'); if (b) b.click(); });
...
await desk.goto(`${base}/index.html?village=1&seed=20260718&vseed=7&time=day`, { waitUntil: 'load' });
await ready(desk);
await ev(desk, () => { const s = document.querySelector('[data-make-panel] input.scale'); if (s) { s.value = '2'; s.dispatchEvent(new Event('input', { bubbles: true })); s.dispatchEvent(new Event('change', { bubbles: true })); } });
