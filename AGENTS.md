...
`src/village/solar-access.js` is the house-to-house sunlight contract. Parcel placement uses actual variant roof polygons, roof/terrain heights, a conservative 30° low-winter-sun angle, and a 1.5m window target; the spatial grids reject only buildings that can physically shadow that target, rather than reserving the whole vegetation corridor as empty land. Frontage, infill, satellites, and residential/government reserved cores participate as targets; a palace precinct is an obstruction but its large vegetation corridor is not misread as one residential window. `check:layout` verifies every planned pair.

`src/village/pavilion-plan.js` owns the public pavilion as an actual building footprint, not a point prop. Required guardian trees and paddies reserve their stable footprints first; the pavilion then reserves its full 4.5m roof radius while avoiding parcels, roads, streams, production land, guardian canopies, every parcel's `solarAccess`, and the exact `planParcelFocus()` sightline. Because a pavilion is a tall public landmark rather than an ordinary fitted house, it conservatively consumes the full opening. Residential focus cameras must aim at the door while their XZ ray remains inside that same south-light opening; never repair a blocked pavilion or house with a fixed render-only camera offset.
