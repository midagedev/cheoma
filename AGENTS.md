...
**Rendering — the flagship look (`src/env/post.js`)**: a unified EffectComposer, on by default (`?post=0` disables). The default PBR pass order is Render → Grade/Rim → Bloom → Bokeh → Flare → Outline → Output. Product ink mode lazily inserts Ink between Outline and Output, crossfades in linear HDR, and sleeps the covered PBR effects at full ink. Output stays last and unique so ACES tone mapping and sRGB conversion happen once. The signature PBR look is golden-hour backlit rim + bloom haze. The rim is a **Fresnel material patch** (`src/env/rim.js`) applied to role-tagged materials, not a screen-space normal pass. Gate product ink changes with `npm run check:ink:app`; the art and reuse contract lives in `docs/ink-landscape.md`.
